...
def upgrade() -> None:
    ...
    # 使用原生SQL安全删除（IF EXISTS），避免在生产环境中因对象不存在而失败
    import sqlalchemy as sa
    from alembic import op
    ...
    # 安全删除冗余表和索引（使用 IF EXISTS）
    # 这些表可能在开发环境存在，但生产环境可能从未创建
    redundant_tables = [
        "statistics_hourly",
        "behaviors",
        "detection_zones",
    ...
    # 安全删除可能存在的索引（表已删除，索引会级联删除，但为了安全还是显式处理）
    redundant_indexes = [
        ("idx_stats_camera_hour", "statistics_hourly"),
        ("idx_stats_hour", "statistics_hourly"),
        ("idx_behaviors_detection_id", "behaviors"),
        ("idx_behaviors_type", "behaviors"),
        ("idx_zones_camera_id", "detection_zones"),
        ("idx_statistics_camera_date", "statistics"),
        ("idx_detections_camera_id", "detections"),
        ("idx_detections_detected_at", "detections"),
        ("idx_detections_type", "detections"),
        ("idx_alerts_camera_id", "alerts"),
        ("idx_alerts_created_at", "alerts"),
        ("idx_alerts_status", "alerts"),
    ]

    for index_name, table_name in redundant_indexes:
