...
def run_detection(args, logger):
    ...
    try:
        ...
        proc_cfg = ProcessConfig(
            ...
            min_dwell_seconds_stand=float((proc_cfg_src.get("min_dwell_seconds", {}) or {}).get("stand", 3)),
            min_dwell_seconds_sink=float((proc_cfg_src.get("min_dwell_seconds", {}) or {}).get("sink", 3)),
            min_dwell_seconds_dryer=float((proc_cfg_src.get("min_dwell_seconds", {}) or {}).get("dryer", 3)),
            cooldown_seconds=float(proc_cfg_src.get("cooldown_seconds", 10)),
            region_entrance=str((proc_cfg_src.get("region_names", {}) or {}).get("entrance", "入口线")),
            region_stand=str((proc_cfg_src.get("region_names", {}) or {}).get("stand", "洗手站立区域")),
            region_sink=str((proc_cfg_src.get("region_names", {}) or {}).get("sink", "洗手池区域")),
            region_dryer=str((proc_cfg_src.get("region_names", {}) or {}).get("dryer", "烘干区域")),
            region_work=str((proc_cfg_src.get("region_names", {}) or {}).get("work", "工作区区域")),
            handwash_min_consecutive=int(proc_cfg_src.get("handwash_min_consecutive", 3)),
        )
        ...
        try:
            ...
            regions_file = args.regions_file or str(os.environ.get('HBD_REGIONS_FILE') or '')
            if not regions_file:
                    ...
                    if _rm is not None:
                        ...
                        if not globals().get('_regions_logged_always', False):
                            ...
                            try:
                                ...
                                pass
                            logger.info(f"[OSD] Regions loaded: count={len(getattr(_rm,'regions',{}))} names={_names}")
                            globals()['_regions_logged_always'] = True
                        # 首帧覆盖图（使用原始帧尺寸坐标系）
                        if frame_idx == 1 and not globals().get('_overlay_first_saved', False):
                            import numpy as _np
                            import cv2 as _cv2
                            _overlay = frame.copy()
                            for _rid, _r in _rm.regions.items():
                                _pts = _np.array([(int(px), int(py)) for (px, py) in _r.polygon], dtype=_np.int32)
                                _cv2.polylines(_overlay, [_pts], isClosed=True, color=(0,255,0), thickness=2)
                                if len(_pts) > 0:
                                    _cv2.putText(_overlay, (_r.name or _rid), (int(_pts[0][0]), int(_pts[0][1]) - 5), _cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
                            Path('logs').mkdir(parents=True, exist_ok=True)
                        ...
                        if args.osd_regions:
                            ...
                            for _rid, _r in _rm.regions.items():
                                _pts = _np.array([(int(px * sx), int(py * sy)) for (px, py) in _r.polygon], dtype=_np.int32)
                                cv2.polylines(annotated, [_pts], isClosed=True, color=(0,255,0), thickness=2)
                                if len(_pts) > 0:
                                    cv2.putText(annotated, (_r.name or _rid), (int(_pts[0][0]), int(_pts[0][1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
                    # 将区域管理器挂到全局供后续使用
                ...
                if process_engine.cfg.enable:
                        ...
                        if not globals().get('_overlay_saved', False):
                            ...
                            if rm is not None:
                                ...
                                for _rid, _r in rm.regions.items():
                                    pts = _np.array([(int(px), int(py)) for (px, py) in _r.polygon], dtype=_np.int32)
                                    color = (0, 255, 0)
                                    _cv2.polylines(overlay, [pts], isClosed=True, color=color, thickness=2)
                                    if len(pts) > 0:
                                        _cv2.putText(overlay, (_r.name or _rid), (int(pts[0][0]), int(pts[0][1]) - 5), _cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
                                Path('logs').mkdir(parents=True, exist_ok=True)
                                _cv2.imwrite('logs/overlay_debug.jpg', overlay)
                                globals()['_overlay_saved'] = True
                                logger.info("Saved region overlay to logs/overlay_debug.jpg")
                    except Exception:
                        pass
                    ...
                    try:
                        if args.osd_regions:
                            ...
                            if rm is not None:
                                ...
                                for _rid, _r in rm.regions.items():
                                    pts = _np.array([(int(px * sx), int(py * sy)) for (px, py) in _r.polygon], dtype=_np.int32)
                                    cv2.polylines(annotated, [pts], isClosed=True, color=(0,255,0), thickness=2)
                                    if len(pts) > 0:
                                        cv2.putText(annotated, (_r.name or _rid), (int(pts[0][0]), int(pts[0][1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        try:
                            ...
                            if _rm_ref is not None:
                                for _rid, _r in _rm_ref.regions.items():
                                    if (_r.name or _rid) == process_engine.cfg.region_sink:
                                        sink_region_obj = _r
                                        break
                        except Exception:
                            sink_region_obj = None
                        ...
                        for tr in tracks:
                            ...
                            try:
                                rm = globals().get('region_manager')
                                if rm is not None and isinstance(tb, (list, tuple)) and len(tb) >= 4:
                                    cur_regions = rm.update_track_regions(tid_assigned, [int(tb[0]), int(tb[1]), int(tb[2]), int(tb[3])])
                                    if cur_regions:
                                        rid = cur_regions[0]
                                        region_name = rm.regions.get(rid).name if rid in rm.regions else rid
                            except Exception:
                                region_name = None
                            # 发网结果匹配
                            has_hairnet = False
                            try:
                                for hr in hairnet_results:
                                    ...
                                    if _bbox_overlap([int(tb[0]), int(tb[1]), int(tb[2]), int(tb[3])], pb, thr=0.1):
                            ...
                            try:
                                if sink_region_obj is not None:
                                    hand_regions = pipeline.get_hand_regions_for_person(
                                        frame, [int(tb[0]), int(tb[1]), int(tb[2]), int(tb[3])]
                                    )
                                    if hand_regions:
                                        ...
                                        for hres in hand_regions:
                                            ...
                                            if not lms:
                                                continue
                                            for lm in lms:
                                                px = int(float(lm.get("x", 0.0)) * float(w_img))
                                                py = int(float(lm.get("y", 0.0)) * float(h_img))
                                                pts_total += 1
                                                try:
                                                    if sink_region_obj.point_in_region((px, py)):
                                                        pts_inside += 1
                                                except Exception:
                                                    pass
                                        if pts_total > 0:
                                            ratio = float(pts_inside) / float(pts_total)
                                            hand_in_sink = (pts_inside >= 3) and (ratio >= 0.2)
                            except Exception:
                                hand_in_sink = False
                            uod = UODPerson(
                    ...
                    try:
                        if frame_idx <= 240:
                            for _p in uod_persons:
                                logger.info(f"track={_p['track_id']} region={_p.get('region')} bbox={_p.get('bbox')} hairnet={_p.get('has_hairnet')}")
                    except Exception:
                        pass
                        ...
                        with events_path.open("a", encoding="utf-8") as f:
                            for ev in events:
                                ...
                                try:
                                    ...
                                    ctx = {}
                                    for _p in uod_persons:
                                        if int(_p.get("track_id", -1)) == int(ev.track_id):
                                            ctx = {
                                    ...
                                    frame_for_save = annotated if annotated is not None else frame
                                    # 传入帧缓冲（转换为 deque 以符合类型）
                                    from collections import deque
                                    _buf = deque(frame_buffer, maxlen=frame_buffer_maxlen)
