...
- **需求**: 需要创建 `DetectionServiceDomain.get_cameras()` 方法
- **依赖**: `ICameraRepository` 接口（已有）

### ⏳ 7. `GET /api/v1/cameras/{camera_id}/stats`
- **状态**: 待完善
- **文件**: `src/api/routers/cameras.py`
- **需求**: 已有 `DetectionServiceDomain.get_camera_analytics()` 方法，需要集成灰度开关
