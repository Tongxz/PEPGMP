...
#### 3.3 `scripts/tools/check_video_stream_status.sh`

| 行号 | 命令 | 说明 |
|------|------|------|
| 12 | `docker exec pyt-redis-dev ...` | 检查Redis状态 |
| 16 | `docker exec pyt-redis-dev ...` | 检查视频流状态 |
...
#### 3.4 `scripts/tests/test_deployment_service.py`

| 行号 | 代码 | 说明 |
|------|------|------|
| 86 | `"container_name": "pyt-postgres-dev"` | 测试用例中的容器名 |

**说明**: 测试脚本中的示例容器名
**建议**: ⚠️ 测试用例，建议更新为实际使用的容器名

---

#### 3.5 `scripts/deploy_prod.sh`
...
3. **脚本**
   - `scripts/tools/check_video_stream_status.sh` - 硬编码容器名
   - `scripts/tests/test_deployment_service.py` - 测试用例
