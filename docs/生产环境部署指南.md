# 生产环境部署指南

> 本文档适用于从 macOS 开发环境部署到 WSL2 Ubuntu / 原生 Ubuntu 生产环境的场景。

---

## 📋 目录

1. [环境要求](#环境要求)
2. [部署架构](#部署架构)
3. [详细部署步骤](#详细部署步骤)
4. [常见问题与解决方案](#常见问题与解决方案)
5. [运维命令参考](#运维命令参考)
6. [故障排查](#故障排查)

---

## 环境要求

### 开发机（构建环境）

| 组件 | 版本要求 | 说明 |
|------|---------|------|
| Docker Desktop | 4.0+ | 用于构建镜像 |
| Bash | 4.0+ | 执行构建脚本 |
| Git | 2.0+ | 代码版本管理 |

### 生产服务器

| 组件 | 版本要求 | 说明 |
|------|---------|------|
| Ubuntu | 22.04 LTS | 或 WSL2 Ubuntu |
| Docker | 24.0+ | 容器运行时 |
| Docker Compose | 2.0+ | 容器编排 |
| 1Panel | 可选 | 可视化管理面板 |

### 硬件要求

| 资源 | 最低配置 | 推荐配置 |
|------|---------|---------|
| CPU | 4 核 | 8 核 |
| 内存 | 8 GB | 16 GB |
| 磁盘 | 50 GB | 100 GB |
| GPU | 无 | NVIDIA GPU（推理加速） |

---

## 部署架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              生产环境架构                                    │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌─────────────┐     ┌─────────────┐     ┌─────────────┐                  │
│   │   Nginx     │────▶│  Frontend   │     │  Prometheus │                  │
│   │   (反向代理) │     │  (Vue3)     │     │  (监控)     │                  │
│   └──────┬──────┘     └─────────────┘     └─────────────┘                  │
│          │                                                                  │
│          ▼                                                                  │
│   ┌─────────────┐     ┌─────────────┐     ┌─────────────┐                  │
│   │    API      │────▶│  PostgreSQL │     │   Redis     │                  │
│   │  (FastAPI)  │     │  (数据库)   │     │  (缓存)     │                  │
│   └─────────────┘     └─────────────┘     └─────────────┘                  │
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │  Volume 挂载:                                                        │  │
│   │  • ./config:/app/config:ro      (只读配置)                          │  │
│   │  • ./models:/app/models:ro      (只读模型)                          │  │
│   │  • ./scripts/init_db.sql        (数据库初始化)                      │  │
│   │  • app_logs:/app/logs           (日志持久化)                        │  │
│   │  • postgres_prod_data           (数据库持久化)                      │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 详细部署步骤

### 第一阶段：开发机准备（macOS）

#### 步骤 1.1：构建 Docker 镜像

```bash
cd /path/to/Pyt

# 使用日期作为版本号构建镜像
bash scripts/build_prod_only.sh 20251202

# 构建成功后会显示：
# [SUCCESS] 后端API镜像构建完成
# [SUCCESS] 前端镜像构建完成
```

**输出的镜像：**
- `pepgmp-backend:20251202`
- `pepgmp-backend:latest`
- `pepgmp-frontend:20251202`
- `pepgmp-frontend:latest`

#### 步骤 1.2：导出镜像为 tar 文件

```bash
# 创建导出目录
mkdir -p docker-images

# 导出后端镜像
docker save pepgmp-backend:20251202 -o docker-images/pepgmp-backend-20251202.tar

# 导出前端镜像
docker save pepgmp-frontend:20251202 -o docker-images/pepgmp-frontend-20251202.tar

# 查看文件大小
ls -lh docker-images/
# pepgmp-backend-20251202.tar    约 2-3 GB
# pepgmp-frontend-20251202.tar   约 50 MB
```

#### 步骤 1.3：同步配置到部署目录

```bash
# 运行配置同步脚本
bash scripts/prepare_minimal_deploy.sh ~/projects/Pyt

# 脚本会自动执行以下操作：
# ✓ 创建目录结构
# ✓ 复制 docker-compose.prod.yml
# ✓ 复制 config/ 目录
# ✓ 复制 models/ 目录（增量同步）
# ✓ 生成 nginx/nginx.conf
# ✓ 复制部署脚本
# ✓ 提示生成 .env.production（首次部署时）
```

#### 步骤 1.4：传输文件到生产服务器

**方法 A：WSL2 共享文件系统**
```bash
# 如果生产环境是 WSL2，可以通过 /mnt/ 访问 Windows 文件
# 将文件复制到 Windows 可访问的位置
cp -r docker-images /mnt/c/Users/YourName/
cp -r ~/projects/Pyt /mnt/c/Users/YourName/
```

**方法 B：SCP/RSYNC**
```bash
# 使用 scp
scp -r docker-images user@server:~/

# 使用 rsync（推荐，支持增量传输）
rsync -avz --progress docker-images/ user@server:~/docker-images/
rsync -avz --progress ~/projects/Pyt/ user@server:~/projects/Pyt/
```

**方法 C：U盘/移动硬盘**
```bash
# 对于离线环境，复制到移动存储设备
cp -r docker-images /Volumes/USB/
cp -r ~/projects/Pyt /Volumes/USB/
```

---

### 第二阶段：生产服务器部署（WSL2/Ubuntu）

#### 步骤 2.1：安装必要工具

```bash
# 更新包列表
sudo apt-get update

# 安装必要工具
sudo apt-get install -y \
    dos2unix \
    rsync \
    curl \
    jq

# 验证 Docker 已安装
docker --version
docker compose version
```

#### 步骤 2.2：修复脚本行尾符（仅 WSL2 需要）

```bash
# 如果脚本从 Windows 文件系统运行，需要修复行尾符
cd /mnt/f/code/PythonCode/Pyt  # 或你的项目路径

# 修复所有 shell 脚本
dos2unix scripts/*.sh scripts/lib/*.sh
```

#### 步骤 2.3：导入 Docker 镜像

```bash
# 进入镜像目录
cd ~/docker-images  # 或你存放 tar 文件的位置

# 导入后端镜像
docker load -i pepgmp-backend-20251202.tar

# 导入前端镜像
docker load -i pepgmp-frontend-20251202.tar

# 验证镜像已导入
docker images | grep pepgmp
# 应显示：
# pepgmp-backend    20251202    xxx    2.5GB
# pepgmp-frontend   20251202    xxx    50MB
```

#### 步骤 2.4：配置部署目录

```bash
# 进入部署目录
cd ~/projects/Pyt

# 如果是首次部署，生成配置文件
bash scripts/generate_production_config.sh

# 脚本会提示输入：
# - API 端口 [8000]
# - 管理员用户名 [admin]
# - CORS 配置 [*]
# - 镜像版本 [latest]
#
# 并自动生成强随机密码
```

#### 步骤 2.5：更新镜像版本号

```bash
cd ~/projects/Pyt

# 更新 .env.production 中的 IMAGE_TAG
bash scripts/update_image_version.sh 20251202

# 验证版本号已更新
grep "IMAGE_TAG" .env.production
# 应显示：IMAGE_TAG=20251202
```

#### 步骤 2.6：验证配置

```bash
cd ~/projects/Pyt

# 验证 Docker Compose 配置是否正确
docker compose -f docker-compose.prod.yml --env-file .env.production config

# 检查镜像是否存在
docker images | grep -E "pepgmp-(backend|frontend)"

# 检查配置文件
ls -la config/
ls -la models/
ls -la nginx/
```

#### 步骤 2.7：启动服务

**方法 A：命令行启动**
```bash
cd ~/projects/Pyt

# 启动所有服务（后台运行）
docker compose -f docker-compose.prod.yml --env-file .env.production up -d

# 查看启动状态
docker compose -f docker-compose.prod.yml ps

# 查看日志
docker compose -f docker-compose.prod.yml logs -f
```

**方法 B：1Panel 启动**
1. 打开 1Panel 管理界面
2. 进入「容器」→「Compose」
3. 点击「创建 Compose 项目」
4. 填写配置：
   - **名称**：`pepgmp-prod`
   - **路径**：`/home/pep/projects/Pyt`（你的部署目录）
   - **Compose 文件**：`docker-compose.prod.yml`
5. 点击「环境变量」，选择「从文件加载」→ `.env.production`
6. 点击「部署」

#### 步骤 2.8：验证部署

```bash
# 等待服务启动（约 30-60 秒）
sleep 30

# 检查容器状态
docker ps

# 健康检查
curl http://localhost:8000/api/v1/monitoring/health

# 检查前端（通过 Nginx）
curl http://localhost/

# 检查数据库连接
docker exec pepgmp-api-prod python -c "from src.database.connection import get_pool; print('DB OK')"
```

---

## 常见问题与解决方案

### 问题 1：Shell 脚本报错 `$'\r': command not found`

**原因**：脚本文件使用了 Windows 行尾符（CRLF）而非 Unix 行尾符（LF）

**解决方案**：
```bash
# 安装 dos2unix
sudo apt-get install -y dos2unix

# 修复所有脚本
dos2unix scripts/*.sh scripts/lib/*.sh

# 或使用 sed
sed -i 's/\r$//' scripts/prepare_minimal_deploy.sh
```

---

### 问题 2：Docker 镜像找不到

**错误信息**：
```
Error response from daemon: No such image: pepgmp-backend:20251202
```

**解决方案**：
```bash
# 检查镜像是否导入
docker images | grep pepgmp

# 如果没有，重新导入
docker load -i pepgmp-backend-20251202.tar

# 检查 .env.production 中的版本号是否匹配
grep IMAGE_TAG .env.production
```

---

### 问题 3：数据库连接失败

**错误信息**：
```
connection refused / Connection timed out
```

**可能原因**：
1. 数据库容器未启动
2. 环境变量配置错误
3. 网络配置问题

**解决方案**：
```bash
# 检查数据库容器状态
docker ps | grep postgres

# 查看数据库日志
docker logs pepgmp-postgres-prod

# 验证数据库连接配置
grep DATABASE .env.production

# 测试数据库连接
docker exec pepgmp-postgres-prod pg_isready -U pepgmp_prod -d pepgmp_production
```

---

### 问题 4：Redis 健康检查失败

**错误信息**：
```
NOAUTH Authentication required
```

**原因**：Redis 健康检查未使用密码

**解决方案**：
这个问题已在 `docker-compose.prod.yml` 中修复。确保使用最新版本的配置文件：
```yaml
healthcheck:
  test: ["CMD-SHELL", "redis-cli -a $${REDIS_PASSWORD} ping | grep -q PONG"]
```

---

### 问题 5：Nginx 配置错误

**错误信息**：
```
nginx: [emerg] open() "/etc/nginx/nginx.conf" failed
```

**解决方案**：
```bash
# 检查 nginx 目录结构
ls -la ~/projects/Pyt/nginx/

# 确保 nginx.conf 是文件而非目录
file ~/projects/Pyt/nginx/nginx.conf

# 如果是目录，删除并重新生成
rm -rf ~/projects/Pyt/nginx/nginx.conf
bash scripts/prepare_minimal_deploy.sh ~/projects/Pyt yes
```

---

### 问题 6：模型文件未挂载

**症状**：API 启动但检测功能不工作

**解决方案**：
```bash
# 检查模型目录
ls -la ~/projects/Pyt/models/

# 确认模型文件存在
find ~/projects/Pyt/models -name "*.pt" -o -name "*.onnx"

# 检查容器内模型目录
docker exec pepgmp-api-prod ls -la /app/models/
```

---

### 问题 7：权限问题

**错误信息**：
```
Permission denied / Operation not permitted
```

**解决方案**：
```bash
# 修复目录权限
sudo chown -R $USER:$USER ~/projects/Pyt

# 设置脚本执行权限
chmod +x ~/projects/Pyt/scripts/*.sh

# 设置配置文件权限
chmod 600 ~/projects/Pyt/.env.production
```

---

### 问题 8：端口被占用

**错误信息**：
```
bind: address already in use
```

**解决方案**：
```bash
# 查找占用端口的进程
sudo lsof -i :8000
sudo lsof -i :80

# 停止占用端口的进程
sudo kill -9 <PID>

# 或修改 .env.production 使用其他端口
API_PORT=8001
```

---

### 问题 9：磁盘空间不足

**错误信息**：
```
no space left on device
```

**解决方案**：
```bash
# 检查磁盘空间
df -h

# 清理 Docker 未使用的资源
docker system prune -a

# 清理旧镜像
docker image prune -a

# 清理旧日志
sudo truncate -s 0 /var/log/*.log
```

---

### 问题 10：容器内存不足

**错误信息**：
```
OOMKilled / Out of memory
```

**解决方案**：
```bash
# 检查容器资源使用
docker stats

# 修改 docker-compose.prod.yml 增加内存限制
# deploy:
#   resources:
#     limits:
#       memory: 8G  # 增加到 8GB
```

---

## 运维命令参考

### 服务管理

```bash
# 启动所有服务
docker compose -f docker-compose.prod.yml --env-file .env.production up -d

# 停止所有服务
docker compose -f docker-compose.prod.yml down

# 重启单个服务
docker compose -f docker-compose.prod.yml restart api

# 查看服务状态
docker compose -f docker-compose.prod.yml ps

# 查看日志
docker compose -f docker-compose.prod.yml logs -f api
docker compose -f docker-compose.prod.yml logs --tail=100 api
```

### 数据库管理

```bash
# 进入数据库容器
docker exec -it pepgmp-postgres-prod psql -U pepgmp_prod -d pepgmp_production

# 备份数据库
docker exec pepgmp-postgres-prod pg_dump -U pepgmp_prod pepgmp_production > backup.sql

# 恢复数据库
cat backup.sql | docker exec -i pepgmp-postgres-prod psql -U pepgmp_prod -d pepgmp_production
```

### 日志管理

```bash
# 查看 API 日志
docker exec pepgmp-api-prod cat /app/logs/error.log

# 实时查看日志
docker logs -f pepgmp-api-prod

# 导出日志
docker logs pepgmp-api-prod > api.log 2>&1
```

### 镜像更新

```bash
# 1. 在开发机构建新镜像
bash scripts/build_prod_only.sh 20251203

# 2. 导出镜像
docker save pepgmp-backend:20251203 -o docker-images/pepgmp-backend-20251203.tar

# 3. 传输到生产服务器
rsync -avz docker-images/ user@server:~/docker-images/

# 4. 在生产服务器导入
docker load -i pepgmp-backend-20251203.tar

# 5. 更新版本号
bash scripts/update_image_version.sh 20251203

# 6. 重启服务
docker compose -f docker-compose.prod.yml --env-file .env.production up -d
```

---

## 故障排查

### 诊断命令

```bash
# 检查容器状态
docker ps -a

# 检查容器日志
docker logs pepgmp-api-prod --tail=50

# 检查容器资源使用
docker stats --no-stream

# 检查网络连接
docker network ls
docker network inspect pepgmp_backend

# 进入容器调试
docker exec -it pepgmp-api-prod bash

# 检查环境变量
docker exec pepgmp-api-prod env | grep -E "DATABASE|REDIS|API"
```

### 健康检查

```bash
# API 健康检查
curl -s http://localhost:8000/api/v1/monitoring/health | jq .

# 数据库健康检查
docker exec pepgmp-postgres-prod pg_isready -U pepgmp_prod

# Redis 健康检查
docker exec pepgmp-redis-prod redis-cli -a $REDIS_PASSWORD ping

# 前端健康检查
curl -s http://localhost/health
```

### 性能监控

```bash
# 查看容器 CPU/内存使用
docker stats

# 查看容器详细信息
docker inspect pepgmp-api-prod

# 查看磁盘使用
docker system df
```

---

## 附录

### 部署检查清单

- [ ] Docker 和 Docker Compose 已安装
- [ ] 镜像已导入（`docker images | grep pepgmp`）
- [ ] 配置文件已生成（`.env.production`）
- [ ] 版本号已更新（`IMAGE_TAG`）
- [ ] 模型文件已复制（`models/`）
- [ ] 配置文件已复制（`config/`）
- [ ] Nginx 配置正确（`nginx/nginx.conf`）
- [ ] 数据库初始化脚本存在（`scripts/init_db.sql`）
- [ ] 所有服务已启动（`docker compose ps`）
- [ ] 健康检查通过

### 重要文件路径

| 文件 | 路径 | 说明 |
|------|------|------|
| Docker Compose | `docker-compose.prod.yml` | 服务编排配置 |
| 环境变量 | `.env.production` | 密码和配置 |
| 凭据备份 | `.env.production.credentials` | 密码备份（部署后删除） |
| Nginx 配置 | `nginx/nginx.conf` | 反向代理配置 |
| 数据库初始化 | `scripts/init_db.sql` | 表结构和初始数据 |
| 应用配置 | `config/cameras.yaml` | 摄像头配置 |
| 模型文件 | `models/*.pt` | AI 模型 |

---

*文档版本：1.0*
*最后更新：2025-12-02*
