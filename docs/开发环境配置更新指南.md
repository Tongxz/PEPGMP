...
**更新时间**: 2025-12-04
**问题**: 容器名和数据库用户名已更新为 `pepgmp-*`，但 `.env` 文件仍使用旧配置
...
1. **数据库认证失败**:
   ...
   ```
   - `.env` 文件仍使用：`pyt_dev` / `pyt_development`
   - 容器已配置为：`pepgmp_dev` / `pepgmp_development`

2. **Redis 认证失败**:
   ```
   ERROR: invalid username-password pair or user is disabled
   ```
   - `.env` 文件仍使用：`pyt_dev_redis`
   - 容器已配置为：`pepgmp_dev_redis`
...
确认 `docker-compose.yml` 中的配置（已更新）:
...
# 重新启动容器（会自动创建新的数据库和用户）
docker-compose up -d database
```

### 2. Redis 数据

Redis 数据通常在内存中，重新启动容器即可。
...
| 数据库名 | `pyt_development` | `pepgmp_development` |
| 数据库密码 | `pyt_dev_password` | `pepgmp_dev_password` |
| Redis 密码 | `pyt_dev_redis` | `pepgmp_dev_redis` |
| 容器名（PostgreSQL） | `pyt-postgres-dev` | `pepgmp-postgres-dev` |
| 容器名（Redis） | `pyt-redis-dev` | `pepgmp-redis-dev` |
