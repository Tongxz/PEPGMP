...
2. **`Pyt`** - 文件系统路径（**本次需要修改**）
   ...
   - 部署目录默认路径：`~/projects/Pyt`, `~/projects/Pyt-deploy`
   - 脚本中的默认路径参数
   - **这些路径需要更新为新目录名**
...
- **行 31**: `SOURCE_PATH="${1:-/mnt/c/Users/$USER/Code/Pyt}"`
- **行 32**: `TARGET_PATH="${2:-$HOME/projects/Pyt}"`
- **行 47**: 示例路径
- **修改为**: 将所有 `Pyt` 替换为 `<新名称>`

**文件**: `scripts/deploy_to_wsl2.sh`
- **行 22**: `DEPLOY_DIR="${3:-~/projects/Pyt}"`
...
- 所有 `~/projects/Pyt` 和 `~/projects/Pyt-deploy` 的引用

**文件**: `docs/WSL直接构建部署指南.md`
- 所有 `~/projects/Pyt` 和 `/mnt/c/Users/.../Code/Pyt` 的引用
...
# 在项目根目录执行
# 替换脚本中的默认路径
find scripts/ -type f -name "*.sh" -exec sed -i.bak 's|~/projects/Pyt|~/projects/<新名称>|g' {} \;
find scripts/ -type f -name "*.sh" -exec sed -i.bak 's|/mnt/c/Users/$USER/Code/Pyt|/mnt/c/Users/$USER/Code/<新名称>|g' {} \;
...
```bash
# 更新文档中的示例路径
find docs/ -type f -name "*.md" -exec sed -i.bak 's|~/projects/Pyt|~/projects/<新名称>|g' {} \;
find docs/ -type f -name "*.md" -exec sed -i.bak 's|~/projects/Pyt-deploy|~/projects/<新名称>-deploy|g' {} \;
...
1. **部署目录**: 如果已有部署目录 `~/projects/Pyt-deploy`，需要手动重命名或更新脚本中的路径
