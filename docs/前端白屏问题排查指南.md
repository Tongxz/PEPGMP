# 前端白屏问题排查指南

## 📋 概述

本文档提供前端白屏问题的系统化排查方法，支持在 WSL2 和 macOS 环境下进行诊断。

---

## 🔍 快速诊断（WSL2 环境）

### 方法 1：使用诊断脚本（推荐）

```bash
# 在 WSL2 中运行
cd ~/projects/Pyt
bash /mnt/c/Users/YourName/Code/PEPGMP/scripts/diagnose_frontend_whitescreen.sh ~/projects/Pyt
```

**脚本会自动检查**：
- ✅ 容器状态
- ✅ 前端静态文件
- ✅ frontend-init 日志
- ✅ Nginx 配置
- ✅ Nginx 日志
- ✅ HTTP 端点
- ✅ HTML 内容
- ✅ Docker Compose 配置

### 方法 2：手动检查步骤

#### 步骤 1：检查容器状态

```bash
cd ~/projects/Pyt
docker compose -f docker-compose.prod.yml ps
```

**预期状态**：
```
pepgmp-frontend-init-prod     Exited (0)  ← 必须成功退出
pepgmp-nginx-prod             Up (healthy)
pepgmp-api-prod              Up (healthy)
pepgmp-postgres-prod         Up (healthy)
pepgmp-redis-prod            Up (healthy)
```

**如果 frontend-init 状态不是 Exited (0)**：
```bash
# 查看详细日志
docker logs pepgmp-frontend-init-prod

# 重新运行 frontend-init
docker compose -f docker-compose.prod.yml up -d frontend-init

# 等待完成
sleep 10

# 再次检查
docker logs pepgmp-frontend-init-prod
```

#### 步骤 2：检查静态文件

```bash
# 检查目录是否存在
ls -la frontend/dist/

# 检查 index.html
ls -la frontend/dist/index.html

# 检查文件数量
find frontend/dist -type f | wc -l

# 检查 assets 目录
ls -la frontend/dist/assets/
```

**预期结果**：
- `frontend/dist/index.html` 存在且大小 > 1KB
- `frontend/dist/assets/` 目录存在
- 至少有几个 `.js` 和 `.css` 文件

**如果文件不存在或为空**：
```bash
# 手动重新提取静态文件
docker compose -f docker-compose.prod.yml up -d frontend-init

# 等待完成
sleep 15

# 检查文件
ls -la frontend/dist/
```

#### 步骤 3：检查 Nginx 配置

```bash
# 检查配置文件是否存在
ls -la nginx/nginx.conf

# 验证 Nginx 配置语法
docker exec pepgmp-nginx-prod nginx -t

# 检查配置内容（应该是 Scheme B）
grep -A 5 "root /usr/share/nginx/html" nginx/nginx.conf
```

**正确配置（Scheme B）**：
```nginx
server {
    root /usr/share/nginx/html;
    index index.html;

    location / {
        try_files $uri $uri/ /index.html;
    }
}
```

**错误配置（Scheme A 残留）**：
```nginx
upstream frontend_backend {
    server frontend:80;  # ❌ 这个不应该存在
}
```

**如果配置错误**：
```bash
# 重新复制正确的配置文件
cp /mnt/c/Users/YourName/Code/PEPGMP/nginx/nginx.conf nginx/nginx.conf

# 重启 Nginx
docker compose -f docker-compose.prod.yml restart nginx
```

#### 步骤 4：检查 Nginx 日志

```bash
# 查看访问日志
docker logs pepgmp-nginx-prod | tail -50

# 查看错误日志
docker logs pepgmp-nginx-prod 2>&1 | grep -i error | tail -20
```

**常见错误**：
- `404 Not Found` - 静态文件路径错误
- `502 Bad Gateway` - API 服务未启动
- `403 Forbidden` - 文件权限问题

#### 步骤 5：测试 HTTP 端点

```bash
# 测试前端
curl -I http://localhost/

# 测试 API
curl http://localhost/api/v1/monitoring/health

# 测试 Nginx 健康检查
curl http://localhost/health
```

**预期结果**：
- 前端返回 `200 OK` 和 HTML 内容
- API 返回 `{"status":"healthy"}`
- 健康检查返回 `healthy`

#### 步骤 6：检查 HTML 内容

```bash
# 获取 HTML 内容
curl http://localhost/ > /tmp/frontend.html

# 检查 HTML 结构
head -20 /tmp/frontend.html

# 检查脚本引用
grep -o 'src="[^"]*"' /tmp/frontend.html | head -5
```

**预期 HTML 结构**：
```html
<!DOCTYPE html>
<html>
<head>
  <script type="module" src="/assets/js/index-xxx.js"></script>
  <link rel="stylesheet" href="/assets/css/index-xxx.css">
</head>
<body>
  <div id="app"></div>
</body>
</html>
```

---

## 🖥️ macOS 环境同步验证

### 方法 1：通过 WSL IP 访问（推荐）

#### 步骤 1：获取 WSL2 IP

在 WSL2 中运行：
```bash
hostname -I | awk '{print $1}'
```

**示例输出**：`172.20.10.2`

#### 步骤 2：在 macOS 浏览器中访问

```
http://172.20.10.2/
```

#### 步骤 3：使用验证脚本

```bash
# 在 macOS 中运行
cd /Users/zhou/Code/Pyt

# 使用验证脚本（需要先获取 WSL IP）
WSL_IP=$(wsl hostname -I | awk '{print $1}')
echo "WSL IP: $WSL_IP"

# 测试端点
curl -I "http://$WSL_IP/"
curl "http://$WSL_IP/api/v1/monitoring/health"
```

### 方法 2：使用远程验证脚本

```bash
# 在 macOS 中运行
cd /Users/zhou/Code/Pyt

# 直接运行（如果 WSL 可以直接访问）
bash scripts/verify_deployment_remote.sh

# 或通过 SSH（如果配置了 SSH）
bash scripts/verify_deployment_remote.sh user@wsl-ip
```

### 方法 3：浏览器开发者工具检查

1. **打开浏览器开发者工具**（F12）
2. **切换到 Console 标签**
3. **检查错误信息**：
   - `Uncaught ReferenceError` - 脚本加载失败
   - `Failed to load resource` - 资源文件 404
   - `CORS error` - 跨域问题

4. **切换到 Network 标签**：
   - 勾选 "Disable cache"
   - 刷新页面（F5）
   - 检查所有请求的状态码
   - 查看失败的请求（红色）

**常见问题**：
- `index.html` 返回 200，但 JS 文件返回 404
  - **原因**：静态文件路径错误
  - **解决**：检查 `frontend/dist/assets/` 目录

- 所有文件返回 200，但页面白屏
  - **原因**：JavaScript 执行错误
  - **解决**：查看 Console 错误信息

---

## 🛠️ 常见问题与解决方案

### 问题 1：frontend-init 容器失败

**症状**：
```bash
docker ps -a | grep frontend-init
# 显示: Exited (1) 或其他非 0 退出码
```

**排查**：
```bash
# 查看详细日志
docker logs pepgmp-frontend-init-prod

# 检查镜像是否存在
docker images | grep pepgmp-frontend

# 检查镜像标签
grep IMAGE_TAG .env.production
```

**解决方案**：
```bash
# 1. 确保镜像标签匹配
cd ~/projects/Pyt
grep IMAGE_TAG .env.production
docker images | grep pepgmp-frontend

# 2. 如果标签不匹配，更新配置
sed -i 's/IMAGE_TAG=.*/IMAGE_TAG=20251202/' .env.production

# 3. 重新运行 frontend-init
docker compose -f docker-compose.prod.yml up -d frontend-init

# 4. 等待完成并检查
sleep 15
docker logs pepgmp-frontend-init-prod
ls -la frontend/dist/
```

### 问题 2：静态文件不存在

**症状**：
```bash
ls frontend/dist/
# 目录不存在或为空
```

**解决方案**：
```bash
# 1. 确保 frontend/dist 目录存在
mkdir -p frontend/dist

# 2. 检查权限
ls -ld frontend/dist

# 3. 重新运行 frontend-init
docker compose -f docker-compose.prod.yml up -d frontend-init

# 4. 等待完成
sleep 15

# 5. 检查文件
ls -la frontend/dist/
find frontend/dist -type f | wc -l
```

### 问题 3：Nginx 配置错误

**症状**：
- Nginx 日志显示 404 或 502
- 浏览器控制台显示资源加载失败

**排查**：
```bash
# 检查 Nginx 配置
docker exec pepgmp-nginx-prod nginx -t

# 查看 Nginx 配置内容
docker exec pepgmp-nginx-prod cat /etc/nginx/nginx.conf | grep -A 10 "server {"
```

**解决方案**：
```bash
# 1. 确保使用正确的配置文件
cp /mnt/c/Users/YourName/Code/PEPGMP/nginx/nginx.conf nginx/nginx.conf

# 2. 验证配置语法
docker exec pepgmp-nginx-prod nginx -t

# 3. 重启 Nginx
docker compose -f docker-compose.prod.yml restart nginx

# 4. 检查日志
docker logs pepgmp-nginx-prod | tail -20
```

### 问题 4：文件权限问题

**症状**：
- 文件存在但无法访问
- Nginx 日志显示 403 Forbidden

**排查**：
```bash
# 检查文件权限
ls -la frontend/dist/
ls -ld frontend/dist

# 检查文件所有者
stat frontend/dist/index.html
```

**解决方案**：
```bash
# 1. 检查当前用户 UID/GID
id -u
id -g

# 2. 更新 .env.production 中的 HOST_UID 和 HOST_GID
nano .env.production
# 设置: HOST_UID=你的UID, HOST_GID=你的GID

# 3. 重新运行 frontend-init（会自动设置权限）
docker compose -f docker-compose.prod.yml up -d frontend-init

# 4. 或手动设置权限
sudo chown -R $(id -u):$(id -g) frontend/dist/
chmod -R 755 frontend/dist/
```

### 问题 5：浏览器缓存问题

**症状**：
- 文件已更新但浏览器仍显示旧内容
- 控制台显示旧的错误信息

**解决方案**：

**Chrome/Edge**：
1. 按 `Ctrl + Shift + Delete`
2. 选择"全部时间"
3. 勾选"缓存的图片和文件"
4. 点击"清除数据"
5. 或使用隐身模式（`Ctrl + Shift + N`）

**Firefox**：
1. 按 `Ctrl + Shift + Delete`
2. 选择"全部"
3. 勾选"缓存"
4. 点击"立即清除"

**或使用开发者工具**：
1. 按 `F12` 打开开发者工具
2. 右键点击刷新按钮
3. 选择"清空缓存并硬性重新加载"

---

## 📊 诊断检查清单

### 容器检查
- [ ] frontend-init 容器状态为 `Exited (0)`
- [ ] nginx 容器状态为 `Up (healthy)`
- [ ] api 容器状态为 `Up (healthy)`
- [ ] database 容器状态为 `Up (healthy)`
- [ ] redis 容器状态为 `Up (healthy)`

### 文件检查
- [ ] `frontend/dist/` 目录存在
- [ ] `frontend/dist/index.html` 存在且大小 > 1KB
- [ ] `frontend/dist/assets/` 目录存在
- [ ] 至少有几个 `.js` 文件
- [ ] 文件权限正确（可读）

### 配置检查
- [ ] `nginx/nginx.conf` 使用 Scheme B 配置
- [ ] Nginx 配置语法正确
- [ ] `docker-compose.prod.yml` 中 nginx 依赖 `frontend-init`
- [ ] `IMAGE_TAG` 与导入的镜像标签匹配

### 网络检查
- [ ] `http://localhost/` 返回 200
- [ ] `http://localhost/api/v1/monitoring/health` 返回 200
- [ ] `http://localhost/health` 返回 200
- [ ] HTML 内容包含有效的脚本标签

### 浏览器检查
- [ ] 浏览器控制台无错误
- [ ] 网络请求全部成功（200）
- [ ] 页面正常渲染（不是白屏）

---

## 🔧 快速修复命令

### 完全重新部署

```bash
cd ~/projects/Pyt

# 1. 停止所有服务
docker compose -f docker-compose.prod.yml down

# 2. 清理静态文件
rm -rf frontend/dist/*

# 3. 重新启动
docker compose -f docker-compose.prod.yml --env-file .env.production up -d

# 4. 等待服务启动
sleep 30

# 5. 检查状态
docker compose -f docker-compose.prod.yml ps

# 6. 验证
curl http://localhost/
```

### 仅修复前端

```bash
cd ~/projects/Pyt

# 1. 停止 Nginx
docker compose -f docker-compose.prod.yml stop nginx

# 2. 清理静态文件
rm -rf frontend/dist/*

# 3. 重新运行 frontend-init
docker compose -f docker-compose.prod.yml up -d frontend-init

# 4. 等待完成
sleep 15

# 5. 检查文件
ls -la frontend/dist/

# 6. 重启 Nginx
docker compose -f docker-compose.prod.yml start nginx

# 7. 验证
curl http://localhost/
```

---

## 📚 相关文档

- [完整部署方案说明](./完整部署方案说明.md) - 架构详细说明
- [WSL2 生产部署详细步骤](./WSL2生产部署详细步骤.md) - 部署步骤
- [竞争条件修复说明](./竞争条件修复说明.md) - Nginx 启动顺序问题

---

**最后更新**: 2025-12-02
