...
def mock_scheduler():
    ...
    # 默认返回值
    scheduler.start_detection = MagicMock(return_value={"ok": True, "pid": 12345, "log": "/tmp/log.txt"})
    scheduler.stop_detection = MagicMock(return_value={"ok": True})
    scheduler.restart_detection = MagicMock(return_value={"ok": True, "pid": 12346, "log": "/tmp/log.txt"})
    scheduler.get_status = MagicMock(return_value={"running": True, "pid": 12345, "log": "/tmp/log.txt"})
    scheduler.status = MagicMock(return_value={"running": True, "pid": 12345, "log": "/tmp/log.txt"})
    scheduler.get_batch_status = MagicMock(return_value={"camera_001": {"running": True, "pid": 12345}})
...
class TestActivateCamera:
    ...
    @pytest.mark.asyncio
    async def test_activate_camera_success(self, camera_control_service, mock_camera_service):
        """测试成功激活摄像头."""
        camera_id = "camera_001"
    ...
    @pytest.mark.asyncio
    async def test_activate_camera_not_found(self, camera_control_service, mock_camera_service):
        """测试激活不存在的摄像头."""
...
class TestDeactivateCamera:
    ...
    @pytest.mark.asyncio
    async def test_deactivate_camera_success_not_running(self, camera_control_service, mock_camera_service, mock_scheduler):
        """测试成功停用摄像头（未运行）."""
        camera_id = "camera_001"
    ...
    @pytest.mark.asyncio
    async def test_deactivate_camera_success_running(self, camera_control_service, mock_camera_service, mock_scheduler):
        """测试成功停用摄像头（正在运行）."""
        camera_id = "camera_001"
    ...
    @pytest.mark.asyncio
    async def test_deactivate_camera_stop_failure(self, camera_control_service, mock_camera_service, mock_scheduler):
        """测试停用摄像头时停止失败（应该继续停用）."""
        camera_id = "camera_001"
        mock_scheduler.status.return_value = {"running": True}
...
class TestToggleAutoStart:
    ...
    @pytest.mark.asyncio
    async def test_toggle_auto_start_enable(self, camera_control_service, mock_camera_service):
        """测试启用自动启动."""
        camera_id = "camera_001"
        mock_camera_service.update_camera.return_value = {"status": "success"}
    ...
    @pytest.mark.asyncio
    async def test_toggle_auto_start_disable(self, camera_control_service, mock_camera_service):
        """测试禁用自动启动."""
        camera_id = "camera_001"
        mock_camera_service.update_camera.return_value = {"status": "success"}
    ...
    @pytest.mark.asyncio
    async def test_toggle_auto_start_not_found(self, camera_control_service, mock_camera_service):
        """测试切换不存在的摄像头的自动启动."""
        camera_id = "camera_999"
        mock_camera_service.update_camera.side_effect = ValueError("摄像头不存在: camera_999")
...
class TestGetCameraLogs:
    ...
    def test_get_camera_logs_success(self, camera_control_service, mock_scheduler, tmp_path):
        """测试成功获取摄像头日志."""
        camera_id = "camera_001"
    ...
    def test_get_camera_logs_not_configured(self, camera_control_service, mock_scheduler):
        """测试日志文件未配置."""
        camera_id = "camera_001"
    ...
    def test_get_camera_logs_file_not_found(self, camera_control_service, mock_scheduler, tmp_path):
        """测试日志文件不存在."""
        camera_id = "camera_001"
