...
class MockDetectionRepository:
    ...
    async def find_by_camera_id(
        ...
        records = [
            r for r in self.records.values() if r.camera_id == camera_id
        ]
...
class TestDetectionServiceDomain:
    ...
    @pytest.mark.asyncio
    async def test_process_detection(
    ...
    @pytest.mark.asyncio
    async def test_process_detection_camera_not_found(
    ...
    @pytest.mark.asyncio
    async def test_get_detection_analytics_no_records(
        self, detection_service_domain
    ):
        """测试获取检测分析报告（无记录）"""
        result = await detection_service_domain.get_detection_analytics(
    ...
    async def test_get_daily_statistics(
        ...
        """测试按天统计事件趋势"""
        result = await detection_service_domain.get_daily_statistics(days=7, camera_id="cam0")

        assert isinstance(result, list)
        assert len(result) <= 7

    @pytest.mark.asyncio
    async def test_get_event_history(
        self, detection_service_domain, sample_records
    ):
        """测试查询事件列表"""
        end_time = datetime.now()
    ...
    async def test_update_violation_status_not_found(
        ...
        """测试更新违规状态（记录不存在）"""
        mock_detection_repository.update_violation_status = AsyncMock(return_value=False)
...
class TestDetectionServiceDomainErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_get_violation_details_empty(
        self, detection_service_domain
    ):
        """测试获取违规明细（空结果）"""
        result = await detection_service_domain.get_violation_details(
...
class TestDetectionServiceDomainEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_get_all_cameras_summary_no_cameras(
        self, detection_service_domain
    ):
        """测试获取所有摄像头统计摘要（无摄像头）"""
        result = await detection_service_domain.get_all_cameras_summary(period="7d")
    ...
    @pytest.mark.asyncio
    async def test_get_realtime_statistics_no_data(
        self, detection_service_domain
    ):
        """测试获取实时统计信息（无数据）"""
        result = await detection_service_domain.get_realtime_statistics()
    ...
    async def test_get_all_cameras_summary_different_periods(
        ...
        with patch.object(
            ...
            # 测试1天
            result1 = await detection_service_domain.get_all_cameras_summary(period="1d")
            assert result1["period"] == "1d"

            # 测试30天
            result2 = await detection_service_domain.get_all_cameras_summary(period="30d")
    ...
    @pytest.mark.asyncio
    async def test_get_daily_statistics_no_records(
        self, detection_service_domain
    ):
        """测试按天统计（无记录）"""
    ...
    @pytest.mark.asyncio
    async def test_get_event_history_no_records(
        self, detection_service_domain
    ):
        """测试查询事件列表（无记录）"""
        end_time = datetime.now()
    ...
    @pytest.mark.asyncio
    async def test_get_recent_history_no_records(
        self, detection_service_domain
    ):
        """测试获取近期事件历史（无记录）"""
        result = await detection_service_domain.get_recent_history(
    ...
    @pytest.mark.asyncio
    async def test_get_camera_stats_detailed_error(
        self, detection_service_domain
    ):
        """测试获取摄像头详细统计信息（错误处理）"""
        with patch.object(
    ...
    @pytest.mark.asyncio
    async def test_get_cameras_all(
        self, detection_service_domain, sample_cameras
    ):
        """测试获取摄像头列表（所有摄像头）"""
        result = await detection_service_domain.get_cameras(active_only=False)
    ...
    async def test_get_camera_analytics_no_performance(
        ...
        assert "camera_info" in result
        assert "performance" not in result or result.get("performance", {}).get("fps_estimate", 0) == 0
    ...
    async def test_get_all_cameras_summary_invalid_period(
        ...
        with patch.object(
            ...
            # 测试无效时间段（应该使用默认7天）
            result = await detection_service_domain.get_all_cameras_summary(period="invalid")
    ...
    async def test_generate_recommendations_with_violations(
        ...
        assert isinstance(recommendations, list)
        # 应该有违规相关的建议
        violation_recommendations = [r for r in recommendations if "违规" in r or "监管" in r]
    ...
    async def test_generate_recommendations_low_frequency(
        ...
        assert isinstance(recommendations, list)
        # 应该有频率相关的建议
        frequency_recommendations = [r for r in recommendations if "频率" in r or "连接" in r]
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_find_by_id_not_found(
        self
    ):
        """测试默认摄像头仓储（查找不存在）"""
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_find_by_region_id(
        self, sample_cameras
    ):
        """测试默认摄像头仓储（按区域查找）"""
        from src.services.detection_service_domain import DefaultCameraRepository
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_delete(
        self, sample_cameras
    ):
        """测试默认摄像头仓储（删除）"""
        from src.services.detection_service_domain import DefaultCameraRepository
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_exists(
        self, sample_cameras
    ):
        """测试默认摄像头仓储（存在检查）"""
        from src.services.detection_service_domain import DefaultCameraRepository
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_count(
        self, sample_cameras
    ):
        """测试默认摄像头仓储（计数）"""
        from src.services.detection_service_domain import DefaultCameraRepository
    ...
    @pytest.mark.asyncio
    async def test_get_detection_service_domain_singleton(
        self
    ):
        """测试获取领域服务（单例模式）"""
    ...
    async def test_get_all_cameras_summary_error_handling_branch(
        ...
        with patch.object(
            ...
            assert "total" in result
            # 验证异常被捕获并处理（至少1个成功）
            camera_results = [v for k, v in result["cameras"].items() if v.get("total_frames", 0) > 0]
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_find_by_id_placeholder_creation(
        self
    ):
    ...
    @pytest.mark.asyncio
    async def test_default_camera_repository_find_by_region_id_empty(
        self
    ):
        """测试默认摄像头仓储（按区域查找，空结果）"""
        from src.services.detection_service_domain import DefaultCameraRepository
    ...
    async def test_get_realtime_statistics_with_violations_metadata(
        ...
        # 为记录添加违规metadata
        for record in sample_records:
            record.add_metadata("violations", [
    ...
    async def test_process_detection_with_violations_loop(
        ...
        with patch.object(
            detection_service_domain, "_publish_event"
        ) as mock_publish:
            result = await detection_service_domain.process_detection(
                camera_id="cam0",
                detected_objects=detected_objects,
