...
class IntegrationTestSuite:
    ...
    def test_endpoint(
        ...
        try:
            ...
            if method == "GET":
                ...
                response = requests.put(url, params=params, json=json_data, timeout=TIMEOUT)
            elif method == "DELETE":
            ...
            if success:
                ...
                result["error"] = response.text[:200] if response.text else f"HTTP {response.status_code} (无响应内容)"
    ...
    def print_summary(self):
        ...
        if failed > 0:
            ...
            # 检查是否有连接错误
            connect_errors = [r for r in self.results if r.get("error_type") == "ConnectionError"]
            if connect_errors:
...
def run_integration_tests():
    ...
    # 3. 违规详情
    suite.test_endpoint(
        "GET", "/api/v1/records/violations/1",
        "获取违规详情"
    )
    
    # 4. 摄像头统计
    suite.test_endpoint(
        "GET", "/api/v1/records/statistics/cam0",
        "获取摄像头统计"
    )
    
    # 5. 检测记录
    suite.test_endpoint(
        "GET", "/api/v1/records/detection-records/cam0",
        "获取检测记录", {"limit": 10}
    )
    ...
    # 18. 更新违规状态
    suite.test_endpoint(
        "PUT", "/api/v1/records/violations/1/status",
        "更新违规状态",
    ...
    # 20. 更新摄像头
    suite.test_endpoint(
        "PUT", "/api/v1/cameras/test_camera_integration",
        "更新摄像头",
