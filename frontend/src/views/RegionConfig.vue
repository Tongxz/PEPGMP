...
// 初始化 Pinia stores（修复 regionStore 未定义报错）
...
const currentRegion = reactive<Region>({
  id: '',
  name: '',
  type: 'detection',
  x: 0,
...
function renderCanvas() {
  ...
  if (isDrawing.value && drawingStart.value) {
    ...
    ctx.lineWidth = 2
    ctx.strokeRect(
      Math.min(drawingStart.value.x, currentRegion.x),
      Math.min(drawingStart.value.y, currentRegion.y),
      Math.abs(currentRegion.x - drawingStart.value.x),
      Math.abs(currentRegion.y - drawingStart.value.y)
    )
    ctx.setLineDash([])
  }
  ctx.restore()
...
function onCanvasMouseDown(e: MouseEvent) {
  if (!selectedCamera.value && !regionStore.backgroundImage) return
  isDrawing.value = true
  const p = getCanvasPos(e)
  drawingStart.value = { ...p }
  currentRegion.x = p.x
  currentRegion.y = p.y
...
function onCanvasMouseMove(e: MouseEvent) {
  ...
  currentRegion.x = p.x
  currentRegion.y = p.y
  currentRegion.width = Math.abs(p.x - drawingStart.value.x)
  currentRegion.height = Math.abs(p.y - drawingStart.value.y)
  // 将 x/y 设为左上角
  const minX = Math.min(drawingStart.value.x, p.x)
  const minY = Math.min(drawingStart.value.y, p.y)
  currentRegion.x = minX
  currentRegion.y = minY
  renderCanvas()
}
...
function onCanvasMouseUp(e: MouseEvent) {
  if (!isDrawing.value || !drawingStart.value) return
  const p = getCanvasPos(e)
  const x = Math.min(drawingStart.value.x, p.x)
  const y = Math.min(drawingStart.value.y, p.y)
  const w = Math.abs(p.x - drawingStart.value.x)
  const h = Math.abs(p.y - drawingStart.value.y)
  isDrawing.value = false
  drawingStart.value = null
  if (w > 3 && h > 3) {
    const newRegion: Region = {
      id: `region_${Date.now()}`,
      name: `区域 ${regions.value.length + 1}`,
      type: 'detection',
    ...
    announceMessage(`已新增区域: ${newRegion.name}`)
  }
  renderCanvas()
}
...
function onCameraChange(value: string) {
  ...
  regions.value = []
  // Here you would typically load the regions for the selected camera
...
// 新增：进入绘制模式（修复 startDrawingMode 未定义）
function startDrawingMode() {
  ...
  isDrawing.value = true
  drawingStart.value = null
  announceMessage('已进入绘制模式：按住鼠标拖拽在画布上创建区域')
  nextTick(() => {
