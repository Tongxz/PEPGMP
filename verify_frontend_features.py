    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if datasets:
                ...
                detail_response = requests.get(f"{API_BASE_URL}/datasets/{dataset_id}", timeout=5)
...
def test_workflows_feature():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            create_response = requests.post(f"{API_BASE_URL}/workflows", json=new_workflow, timeout=30)
            if create_response.status_code == 200:
                ...
                # 测试运行工作流
                run_response = requests.post(f"{API_BASE_URL}/workflows/{workflow_id}/run", timeout=60)
                if run_response.status_code == 200:
                    print(f"    ✅ 运行工作流成功")
                    
                    # 清理：删除测试工作流
                    delete_response = requests.delete(f"{API_BASE_URL}/workflows/{workflow_id}", timeout=10)
