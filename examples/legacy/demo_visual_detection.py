...
class VisualDetectionDemo:
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict]:
        ...
        if self.pose_detector:
            try:
                hand_results = self.pose_detector.detect(frame)
                if hand_results:
        ...
        if self.motion_analyzer and hand_results:
            try:
                ...
                motion_results = {
                    ...
                    if handwash_conf > sanitize_conf
                    else "sanitizing",
    ...
    def draw_visualizations(
        self, frame: np.ndarray, hand_results: List, motion_results: Dict
    ) -> np.ndarray:
        """使用统一可视化管理器绘制可视化信息"""
        if self.visualization_manager:
            ...
            vis_frame = self.visualization_manager.visualize_frame(
                frame, results, self.stats["frames_processed"]
            )
            ...
            # 回退到原始可视化方法
            return self._draw_fallback_visualizations(
                frame, hand_results, motion_results
            )

    def _draw_fallback_visualizations(
        self, frame: np.ndarray, hand_results: List, motion_results: Dict
    ) -> np.ndarray:
        """回退可视化方法（当可视化管理器不可用时）"""
        ...
        if motion_results and motion_results.get("behavior_confidence", 0) > 0.3:
            ...
            cv2.putText(
                frame,
                f"Handwashing: {handwash_conf:.2f}",
                (10, 70),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                self.colors["stats"],
        ...
        if self.behavior_history:
            recent_behavior = max(
                set(self.behavior_history), key=self.behavior_history.count
            )
            cv2.putText(
                frame,
                f"Recent: {recent_behavior}",
                (10, height - 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                self.colors["behavior"],
