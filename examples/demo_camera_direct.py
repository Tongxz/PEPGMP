    ...
    try:
        ...
        try:
            ...
            if rm.load_regions_config(cfg_path):
                ...
                logger.info(f"Regions loaded for demo from {cfg_path}, count={len(region_manager_demo.regions)}")
            else:
                logger.warning("No regions loaded for demo; region-gated logic will be disabled.")
...
class DemoSession:
    def __init__(self, track_max_miss: int = 30, track_iou_thr: float = 0.2,
                 track_dist_thr: float = 200.0, track_strategy: str = 'hungarian', track_iou_weight: float = 0.6,
                 track_recycle_ids: bool = False, track_force_revival: bool = False, track_force_revival_dist: float = 0.0,
                 stable_iou_thr: float = 0.2, stable_dist_thr: float = 480.0, stable_max_ids: int = 2):
        self.tracker = MultiObjectTracker(max_disappeared=track_max_miss, iou_threshold=track_iou_thr,
                                          dist_threshold=track_dist_thr, match_strategy=track_strategy,
                                          iou_weight=track_iou_weight, recycle_ids=track_recycle_ids,
                                          force_revival=track_force_revival,
                                          force_revival_dist=(track_force_revival_dist if track_force_revival_dist > 0 else None))
    ...
    def get_stable_id(self, track_id: int, bbox, frame_idx: int, iou_thr: float = None, dist_thr: float = None):
        if iou_thr is None:
...
# --- Main Camera Loop ---
def parse_args():
    parser = argparse.ArgumentParser(description="Real-time or video human behavior detection demo")
    parser.add_argument("--source", type=str, default="0", help="Camera index (e.g., '0') or video file path")
    parser.add_argument("--width", type=int, default=424, help="Capture width for camera source")
    parser.add_argument("--height", type=int, default=240, help="Capture height for camera source")
    parser.add_argument("--frame-skip", type=int, default=5, help="Process every Nth frame (optimizer)")
    parser.add_argument("--sim-threshold", type=float, default=0.995, help="Frame similarity threshold for skipping")
    parser.add_argument("--hand-only", action="store_true", help="Focus on hand behavior: disable hairnet detection")
    parser.add_argument("--max-frames", type=int, default=0, help="Stop after processing this many frames (0 = unlimited)")
    parser.add_argument("--exit-on", type=str, default="none", choices=["none", "handwash", "sanitize", "no_hairnet", "no_person"], help="Exit when event is observed")
    parser.add_argument("--exit-consecutive", type=int, default=1, help="Consecutive frames required to trigger exit event")
    parser.add_argument("--save-video", type=str, default="", help="Output path to save annotated video (e.g., output/annotated.mp4)")
    parser.add_argument("--save-detail", type=str, default="", help="Path to save per-frame detection details (JSONL)")
    parser.add_argument("--font", type=str, default="/System/Library/Fonts/PingFang.ttc", help="TTF/TTC font path for Chinese OSD text")
    parser.add_argument("--track-iou-thr", type=float, default=0.2, help="Tracker IoU matching threshold (lower=more tolerant)")
    parser.add_argument("--track-dist-thr", type=float, default=220.0, help="Tracker center distance threshold (pixels)")
    parser.add_argument("--track-strategy", type=str, default="hungarian", choices=["hungarian","greedy"], help="Matching strategy")
    parser.add_argument("--track-iou-weight", type=float, default=0.65, help="Fusion weight for IoU vs distance [0..1]")
    parser.add_argument("--track-recycle-ids", action="store_true", help="Recycle deleted track IDs to reduce ID growth")
    parser.add_argument("--track-force-revival", action="store_true", help="Force revival of nearby lost IDs with larger distance")
    parser.add_argument("--track-force-revival-dist", type=float, default=0.0, help="Force revival distance in pixels (0=1.5x dist-thr)")
    # Stable ID 控制
    parser.add_argument("--stable-iou-thr", type=float, default=0.25, help="Stable-ID matching IoU threshold")
    parser.add_argument("--stable-dist-thr", type=float, default=480.0, help="Stable-ID matching center distance threshold")
    parser.add_argument("--stable-max-ids", type=int, default=2, help="Max stable IDs allowed in display/export")
    parser.add_argument("--track-max-miss", type=int, default=30, help="Max missed frames before a track is considered lost")
    parser.add_argument("--smooth-window", type=int, default=5, help="Temporal smoothing window (frames) per track_id")
    parser.add_argument("--handwash-min-frames", type=int, default=3, help="Min consecutive frames to confirm handwash per track")
    parser.add_argument("--sanitize-min-frames", type=int, default=3, help="Min consecutive frames to confirm sanitize per track")
    parser.add_argument("--handwash-neg-frames", type=int, default=5, help="Consecutive negative frames to confirm NOT handwashing for violation")
    parser.add_argument("--dump-handseq", type=str, default="", help="Directory to dump hand-sequence npz for training")
    parser.add_argument("--dump-window", type=int, default=30, help="Sequence window length (frames) for dump")
    parser.add_argument("--dump-step", type=int, default=5, help="Dump every N frames per track when window is full")
    # 新增：日志限流与最简OSD
    parser.add_argument("--log-interval", type=int, default=10, help="Print tracking summary every N processed frames (0=disable)")
    parser.add_argument("--osd-minimal", action="store_true", help="Enable minimal OSD: only bbox and one-line label; no banner")
    parser.add_argument("--draw-regions", action="store_true", help="Draw configured regions on each frame for verification")
    parser.add_argument("--regions-ref-size", type=str, default="", help="Reference canvas size 'WxH' used when annotating regions (e.g., 1280x720)")
    parser.add_argument("--regions-canvas-size", type=str, default="", help="Frontend canvas size 'WxH' used when annotating (e.g., 1280x720)")
    parser.add_argument("--regions-bg-size", type=str, default="", help="Background image size 'WxH' shown in canvas (e.g., 1920x1080)")
    parser.add_argument("--regions-fit-mode", type=str, default="contain", choices=["contain","cover","stretch"], help="How background fits in canvas")
    # 新增：洗手融合模式（站立区 AND (ML洗手 OR 水池子区>=N帧)）
    parser.add_argument("--fusion-handwash", action="store_true", help="Enable fusion: stand area AND (ML handwash OR faucet subregion >= N frames)")
    parser.add_argument("--stand-name", type=str, default="洗手站立区域", help="Name of standing area region for handwash gating")
    parser.add_argument("--faucet-name", type=str, default="洗手水池", help="Name of faucet sub-region for handwash fusion")
    parser.add_argument("--faucet-min-frames", type=int, default=3, help="Min consecutive frames in faucet sub-region to count as washing")
    parser.add_argument("--log-fusion-details", action="store_true", help="Log per-frame fusion details for handwash decision")
    parser.add_argument("--osd-debug-hand", action="store_true", help="Always draw hand boxes/landmarks and ML score for debugging")
...
def main():
    ...
    demo_session = DemoSession(track_max_miss=args.track_max_miss, track_iou_thr=args.track_iou_thr,
                               track_dist_thr=args.track_dist_thr, track_strategy=args.track_strategy,
                               track_iou_weight=args.track_iou_weight, track_recycle_ids=args.track_recycle_ids,
                               ...
                               stable_max_ids=args.stable_max_ids) # Create a session for this demo
    optimizer = None
    if not args.hand_only:
        optimizer = VideoStreamOptimizer(optimized_pipeline, frame_skip=args.frame_skip, similarity_threshold=args.sim_threshold)
    last_annotated = None
    last_result = None
    # Per-track temporal states
    track_states = {}
    # Per-track compliance state machine (lite)
    compliance_state = {}  # track_id -> {hairnet_ok:bool, handwash_ok:bool, sanitize_ok:bool}
    ...
    def _extract_two_hands_vector(hand_regions, person_bbox, image_shape):
        ...
        for hreg in hands_sorted:
            ...
            if not lmks:
                ...
                vec.extend(_normalize_landmarks_to_person(lmks, person_bbox, image_shape))
            hands_taken += 1
    ...
    try:
        ...
        while True:
                        ...
                        if "x" in canv and "x" in bg:
                            try:
                                ...
                                if cw > 0 and ch > 0 and bw > 0 and bh > 0:
                                    region_manager_demo.scale_from_canvas_and_bg(
                                        cw, ch, bw, bh, w0, h0, fit_mode=args.regions_fit_mode
                                    )
                                else:
                            ...
                            if "x" in ref:
                                try:
                                    ...
                                    rw = int(rw); rh = int(rh)
                                    if rw > 0 and rh > 0:
                                        region_manager_demo.scale_from_reference(rw, rh, w0, h0)
                                    else:
                                        region_manager_demo.scale_to_frame_if_needed(w0, h0)
                                except Exception:
                                    region_manager_demo.scale_to_frame_if_needed(w0, h0)
            ...
            if args.hand_only:
                ...
                result = optimizer.process_frame(frame) if optimizer is not None else optimized_pipeline.detect_comprehensive(frame)
            ...
            try:
                if args.log_interval and (frame_read_count % max(1, args.log_interval) == 0):
                    def _sid_str(tobj):
                        sid = demo_session.get_stable_id(
                            tobj['track_id'], tobj['bbox'], frame_read_count,
            ...
            # --- Annotation and Display ---
                ...
                if region_manager_demo is not None and args.draw_regions:
                    try:
                        for r in region_manager_demo.regions.values():
                            ...
                            if len(poly) >= 2:
                                cv2.polylines(annotated, [np.array(poly, dtype=np.int32)], isClosed=True, color=(255, 0, 0), thickness=2)
                                # 在区域中心写名字
                                cx, cy = map(int, r.get_center())
                                cx = max(0, min(cx, annotated.shape[1]-1)); cy = max(0, min(cy, annotated.shape[0]-1))
                                cv2.putText(annotated, (r.name or r.region_type.value), (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
                    except Exception:
                        pass
                frame_violations = []  # Collect OSD messages for this frame
                frame_report = None
                if detail_reports is not None:
                    frame_report = {"frame_idx": frame_read_count, "objects": [], "violations": []}
                ...
                def _queue_text(text, org, font_size=16, color=(255, 255, 255)):
                    _osd_queue.append((str(text), (int(org[0]), int(org[1])), int(font_size), (int(color[0]), int(color[1]), int(color[2]))))
                def _flush_text(img):
                    if not _osd_queue:
                        return img
                    try:
                        ...
                        for text, org, fsize, bgr in _osd_queue:
                            ...
                            draw.text(org, text, font=font, fill=(bgr[2], bgr[1], bgr[0]))
                        _osd_queue.clear()
                        return cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
                    except Exception:
                        # Fallback: 用简化ASCII逐条画（不支持中文会乱码）
                        for text, org, fsize, bgr in _osd_queue:
                            cv2.putText(img, text, org, cv2.FONT_HERSHEY_SIMPLEX, max(0.5, fsize/24.0), bgr, 2)
                        _osd_queue.clear()
                        return img
                for tobj in tracked_objects:
                    bbox = tobj['bbox']
                    ...
                    # 计算稳定ID（对外显示）
                    stable_id = demo_session.get_stable_id(track_id, bbox, frame_read_count, iou_thr=max(0.25, args.track_iou_thr), dist_thr=max(args.track_dist_thr, 320.0))
                    ...
                    inside_faucet = False
                    if region_manager_demo is not None:
                        region_ids = region_manager_demo.update_track_regions(track_id, bbox)
                    ...
                    st = track_states.setdefault(track_id, {"hw_cnt":0, "sz_cnt":0, "hw_false":0, "sz_false":0, "hw_neg_cnt":0, "last_ts":time.time(), "faucet_cnt":0})
                    ...
                    # 仅消毒在 SANITIZE 区域计数
                    is_sz = is_sz_raw and (inside_sanitize if region_manager_demo is not None else True)
                    # 水池子区连续帧计数
                    if inside_faucet:
                    ...
                    if args.fusion_handwash:
                        is_hw_fused = (inside_handwash if region_manager_demo is not None else True) and (
                            bool(is_hw_raw) or (st.get("faucet_cnt", 0) >= max(1, args.faucet_min_frames))
                        )
                        is_hw = is_hw_fused
                    else:
                        # 仅区域门控：HANDWASH 区域内且 ML 洗手为真
                        is_hw = is_hw_raw and (inside_handwash if region_manager_demo is not None else True)
                    # Update counters (bounded by smooth window)
                    st["hw_cnt"] = min(args.smooth_window, st["hw_cnt"] + 1) if is_hw else max(0, st["hw_cnt"] - 1)
                    st["sz_cnt"] = min(args.smooth_window, st["sz_cnt"] + 1) if is_sz else max(0, st["sz_cnt"] - 1)
                    # Only accumulate negative count when inside the corresponding region
                    if region_manager_demo is None or inside_handwash:
                        st["hw_neg_cnt"] = min(args.smooth_window, st["hw_neg_cnt"] + 1) if (not is_hw) else max(0, st["hw_neg_cnt"] - 1)
                    ...
                    if args.log_fusion_details:
                        try:
                            ...
                            pass
                    # 根据洗手状态绘制不同颜色的人员框
                    box_color = (0, 215, 255) if hw_confirm else (0, 255, 0)  # 洗手=黄青色，否则绿色
                    thickness = 3 if hw_confirm else 2
                    cv2.rectangle(annotated, (x1, y1), (x2, y2), box_color, thickness)
                    # 手部可视化与ML置信度调试
                    try:
                        if args.osd_debug_hand or hw_confirm:
                            hand_regions = optimized_pipeline.get_hand_regions_for_person(frame, bbox)
                            # 绘制人体中线
                            cx_mid = int((x1 + x2) / 2)
                            cv2.line(annotated, (cx_mid, y1), (cx_mid, y2), (200, 200, 200), 1)
                            # 仅取前两只手用于可视化
                            centers = []
                            for hreg in (hand_regions or [])[:2]:
                                ...
                                for lm in lmks:
                                    ...
                                    if 0.0 <= float(px) <= 1.0 and 0.0 <= float(py) <= 1.0:
                                        lx = int(float(px) * annotated.shape[1])
                                        ly = int(float(py) * annotated.shape[0])
                                    else:
                                        lx = int(px)
                                        ly = int(py)
                                    if 0 <= lx < annotated.shape[1] and 0 <= ly < annotated.shape[0]:
                                        cv2.circle(annotated, (lx, ly), 2, (0, 215, 255), -1)
                            ...
                            _queue_text(dbg_text, (x1, min(y2 + 18, annotated.shape[0]-5)), font_size=15, color=(255, 255, 0))
                    except Exception:
                        pass
                    # Update compliance state (sequence)
                    comp = compliance_state.setdefault(track_id, {"hairnet_ok": False, "handwash_ok": False, "sanitize_ok": False})
                    ...
                    if not args.hand_only:
                        seq_txt_cn = []
                        if comp.get("hairnet_ok"): seq_txt_cn.append("发网")
                        if comp.get("handwash_ok"): seq_txt_cn.append("洗手")
                        if comp.get("sanitize_ok"): seq_txt_cn.append("消毒")
                        if seq_txt_cn:
                            label += f" | 流程:{'→'.join(seq_txt_cn)}"
                    if args.osd_minimal:
                        # 仅一行标签
                        _queue_text(label, (x1, max(12, y1 - 12)), font_size=16, color=(0, 255, 0))
                    else:
                        _queue_text(label, (x1, max(12, y1 - 12)), font_size=16, color=(0, 255, 0))
                    # Draw NEXT step hint
                    if not args.osd_minimal and not args.hand_only:
                        if next_step and next_step != "enter":
                            next_map = {"hairnet": "发网", "handwash": "洗手", "sanitize": "消毒"}
                            hint = f"下一步: {next_map.get(next_step, next_step)}"
                            _queue_text(hint, (x1, min(y2 + 20, annotated.shape[0]-5)), font_size=17, color=(0, 215, 255))
                        elif next_step == "enter":
                            _queue_text("流程完成", (x1, min(y2 + 20, annotated.shape[0]-5)), font_size=17, color=(0, 255, 0))
                    ...
                    if frame_report is not None:
                        frame_report["objects"].append({
                            ...
                            "hairnet": None if has_hairnet is None else bool(has_hairnet),
                            "handwash": {"raw": bool(is_hw_raw), "smoothed": bool(hw_confirm), "count": int(st["hw_cnt"])},
                            "sanitize": {"raw": bool(is_sz_raw), "smoothed": bool(sz_confirm), "count": int(st["sz_cnt"])},
                            "sequence": {"hairnet_ok": bool(comp.get("hairnet_ok", False)), "handwash_ok": bool(comp.get("handwash_ok", False)), "sanitize_ok": bool(comp.get("sanitize_ok", False))},
                            "next_step": next_step,
                        })
                    ...
                    if args.dump_handseq:
                        try:
                            hand_regions = optimized_pipeline.get_hand_regions_for_person(frame, bbox)
                            vec = _extract_two_hands_vector(hand_regions, bbox, frame.shape)
                            ...
                            if len(buf) > args.dump_window:
                                buf.pop(0)
                            if len(buf) == args.dump_window and (frame_read_count % args.dump_step == 0):
                                # save npz: features (window, feat_dim), labels (window,)
                                feats = np.array([b["feat"] for b in buf], dtype=np.float32)
                                labels = np.array([b["label"] for b in buf], dtype=np.int8)
                                ...
                                np.savez_compressed(out_path, feats=feats, labels=labels, track_id=track_id)
                    ...
                    if not args.hand_only and inside_entrance:
                        for h_res in hairnet_results:
                            if _bbox_overlap(bbox, h_res.get("person_bbox", [])) and not h_res.get("has_hairnet", True):
                                frame_violations.append(f"编号 {stable_id}: 未佩戴发网")
                                cv2.rectangle(annotated, (x1, y1), (x2, y2), (0, 0, 255), 2)
                                break
                    # 2) Not handwashing (negative streak)
                    if (region_manager_demo is None or inside_handwash) and st.get("hw_neg_cnt", 0) >= max(1, args.handwash_neg_frames):
                    ...
                    if inside_work_area and not (comp["hairnet_ok"] and comp["handwash_ok"] and comp["sanitize_ok"]):
                ...
                if args.save_video:
                    if writer is None:
                        ...
                        writer = cv2.VideoWriter(writer_info["path"], fourcc, writer_info["fps"], (w, h))
                ...
                if args.exit_on != "none":
                    ...
                    if args.exit_on == "handwash":
                        ...
                        event_present = any(track_states.get(t['track_id'], {}).get('hw_cnt',0) >= args.handwash_min_frames for t in tracked_objects)
                    elif args.exit_on == "sanitize":
                        event_present = any(track_states.get(t['track_id'], {}).get('sz_cnt',0) >= args.sanitize_min_frames for t in tracked_objects)
                    elif args.exit_on == "no_hairnet":
                        if not args.hand_only:
                            event_present = any(not h.get("has_hairnet", True) for h in hairnet_results)
        ...
        try:
            if detail_reports is not None and len(detail_reports) > 0:
                ...
                logger.info(f"Detailed detection report saved to {args.save_detail} ({len(detail_reports)} frames)")
        except Exception as _:
