...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cleanup output & logs by retention days")
    parser.add_argument("--days", type=int, default=30, help="保留天数（删除早于该天数的文件）")
    parser.add_argument("--paths", nargs="*", default=["output/captures", "logs"], help="要清理的路径列表")
