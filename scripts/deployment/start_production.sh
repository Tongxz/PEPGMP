...
if command -v nvidia-smi &> /dev/null; then
    ...
    echo -e "${YELLOW}⚠️  GPU不可用，将使用CPU运行${NC}"
fi
...
# 提示
echo -e "\n${YELLOW}提示:${NC}"
echo -e "${YELLOW}1. 首次启动会自动转换TensorRT模型（需要2-5分钟）${NC}"
echo -e "${YELLOW}2. 查看日志: docker-compose -f docker-compose.prod.yml logs -f api${NC}"
echo -e "${YELLOW}3. 停止服务: docker-compose -f docker-compose.prod.yml down${NC}"
echo -e "${YELLOW}4. 重启服务: docker-compose -f docker-compose.prod.yml restart${NC}"
