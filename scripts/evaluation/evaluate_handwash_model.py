...
def evaluate_model(
    ...
    metrics["evaluation"] = {
        "accuracy_status": "优秀"
        if accuracy >= 0.90
        else ("良好" if accuracy >= 0.80 else "需要改进"),
        "precision_status": "优秀"
        if precision >= 0.85
        else ("良好" if precision >= 0.75 else "需要改进"),
        "recall_status": "优秀"
        if recall >= 0.85
        else ("良好" if recall >= 0.75 else "需要改进"),
    ...
    print("=" * 50)
    print(
        f"Accuracy:       {accuracy:.4f} ({metrics['evaluation']['accuracy_status']})"
    )
    print(
        f"Precision:      {precision:.4f} ({metrics['evaluation']['precision_status']})"
    )
...
def main():
    ...
    try:
        metrics = evaluate_model(
            args.model, args.data, args.annotations, args.device, args.output
        )
