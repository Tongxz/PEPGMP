...
def check_optimization_status() -> Dict[str, Any]:
    ...
    if hasattr(optimized_pipeline, "enable_state_management"):
        ...
        if status["state_management_enabled"]:
            logger.info("✅ 状态管理已启用")
            if (
                hasattr(optimized_pipeline, "state_manager")
                and optimized_pipeline.state_manager
            ):
    ...
    if hasattr(optimized_pipeline, "enable_cache"):
        ...
        if status["cache_enabled"]:
            logger.info("✅ 缓存已启用")
            if (
                hasattr(optimized_pipeline, "frame_cache")
                and optimized_pipeline.frame_cache
            ):
...
def test_detection_performance(image_path: str, iterations: int = 5) -> Dict[str, Any]:
    ...
    for i in range(iterations):
        ...
        logger.info(
            f"  第 {i+1} 次: {elapsed:.3f}s, 检测到 {len(result.person_detections)} 个人"
        )
...
def test_cache_performance(image_path: str, iterations: int = 10) -> Dict[str, Any]:
    ...
    # 获取缓存统计
    cache_stats = (
        optimized_pipeline.frame_cache.get_stats()
        if optimized_pipeline.frame_cache
        else {}
    )
    ...
    logger.info(
        f"缓存命中率: {pipeline_stats.get('cache_hits', 0) / (pipeline_stats.get('cache_hits', 0) + pipeline_stats.get('cache_misses', 1)) * 100:.2f}%"
    )
...
def generate_report(
    ...
    report.append("")
    report.append(
        f"- FrameMetadata可用: {'✅' if status.get('frame_metadata_available') else '❌'}"
    )
    report.append(f"- 状态管理启用: {'✅' if status.get('state_management_enabled') else '❌'}")
    report.append(
        f"- 异步检测启用: {'✅' if status.get('async_detection_enabled') else 'ℹ️  (默认禁用)'}"
    )
    report.append(f"- 缓存启用: {'✅' if status.get('cache_enabled') else '❌'}")
    report.append(
        f"- ROI优化启用: {'✅' if status.get('roi_optimization_enabled') else '❌'}"
    )
    report.append("")
    ...
    if cache_performance:
        ...
        report.append("")
        report.append(
            f"- 第一次检测时间: {cache_performance.get('first_detection_time', 0):.3f}s"
        )
