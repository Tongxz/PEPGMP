...
# 2. 全图发网检测（作为参考）
print("\n2. 全图发网检测（作为参考）...")
hairnet_full_results = hairnet_model(frame, conf=0.15, iou=0.45, imgsz=640, verbose=False)
...
for i, person in enumerate(person_detections):
    ...
    roi_results = hairnet_model(head_roi_processed, conf=0.15, iou=0.45, imgsz=640, verbose=False)
    ...
    for j, full_hairnet in enumerate(full_hairnet_detections):
        ...
        print(f"       发网 {j+1} (置信度={full_hairnet['confidence']:.3f}): "
...
for person in person_detections:
    ...
    cv2.putText(vis_frame, "Person", (x1, y1-10), 
...
for hairnet in full_hairnet_detections:
    ...
    cv2.putText(vis_frame, f"Hairnet {hairnet['confidence']:.2f}", 
...
for i, person in enumerate(person_detections):
    ...
    cv2.putText(vis_frame, f"ROI {i+1}", (roi_x1, roi_y1-10), 
