...
class ImprovedHeadROIExtractor:
    ...
    def _extract_single_head_roi(
        ...
        try:
            ...
            if "landmark" in landmarks:
                # MediaPipe格式
                return self._extract_from_mediapipe_landmarks(
                    landmarks["landmark"], width, height
                )
            elif "keypoints" in landmarks:
                # OpenPose格式
                return self._extract_from_openpose_landmarks(
                    landmarks["keypoints"], width, height
                )
            else:
    ...
    def _extract_from_mediapipe_landmarks(
    ...
    def _extract_from_openpose_landmarks(
    ...
    def _validate_roi(
        self, roi: Tuple[int, int, int, int], image_width: int, image_height: int
    ) -> bool:
        """
        验证ROI区域是否合理
        ...
        aspect_ratio = w / h
        if (
            aspect_ratio < self.aspect_ratio_range[0]
            or aspect_ratio > self.aspect_ratio_range[1]
        ):
            return False

        return True

    def visualize_head_rois(
        self,
        image: np.ndarray,
        ...
        for i, (x, y, w, h) in enumerate(head_rois):
            ...
            cv2.rectangle(
                vis_image, (x, y - label_size[1] - 5), (x + label_size[0], y), color, -1
            )
            cv2.putText(
                vis_image,
                label,
