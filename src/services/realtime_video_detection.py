...
# 本地模块导入
from src.detection.enhanced_hand_detector import DetectionMode, EnhancedHandDetector
...
class RealtimeVideoDetector:
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        try:
            ...
            # 4. 行为分析
            handwash_confidence = self.motion_analyzer.analyze_handwashing_enhanced(
                track_id
            )
            sanitize_confidence = self.motion_analyzer.analyze_sanitizing_enhanced(
                track_id
            )
            ...
            if self.deep_recognizer:
                motion_summary = self.motion_analyzer.get_enhanced_motion_summary(
                    track_id
                )
                if motion_summary:
    ...
    def _determine_final_behavior(
        self, handwash_conf: float, sanitize_conf: float, deep_preds: Dict[str, float]
    ) -> Tuple[str, float]:
        """确定最终行为"""
        # 传统方法权重
        ...
        # 融合预测
        final_handwash = (
            traditional_weight * handwash_conf
            + deep_weight * deep_preds.get("handwash", 0.0)
        )
        final_sanitize = (
            traditional_weight * sanitize_conf
            + deep_weight * deep_preds.get("sanitize", 0.0)
        )
        ...
        if len(landmarks) >= 21:  # MediaPipe手部有21个关键点
            ...
            for start_idx, end_idx in connections:
                if start_idx < len(landmarks) and end_idx < len(landmarks):
                    ...
                    cv2.line(frame, start_point, end_point, self.colors["landmarks"], 1)
    ...
    def _draw_behavior_info(
        ...
        cv2.putText(
            frame,
            f"洗手: {handwash_conf:.2f}",
            (20, y_offset),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.config.font_scale,
            self.colors["handwash"],
            1,
        )
    ...
    def _draw_statistics(self, frame: np.ndarray) -> None:
        ...
        stats_x = w - 250
        cv2.rectangle(
            frame, (stats_x, 10), (w - 10, 150), self.colors["background"], -1
        )
        ...
        try:
            while True:
                ...
                if self.frame_count % 100 == 0:
                    ...
                    self.logger.info(
                        f"处理进度: {progress:.1f}% ({self.frame_count}/{total_frames})"
                    )
    ...
    try:
        if args.input:
            ...
            statistics = detector.process_video(
                args.input, args.output, display=not args.no_display
            )
            print("\n=== 处理统计 ===")
