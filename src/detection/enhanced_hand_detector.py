...
class EnhancedHandDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"EnhancedHandDetector initialized with mode: {detection_mode.value}"
        )
    ...
    def detect_hands_robust(self, image: np.ndarray) -> List[HandDetectionResult]:
        ...
        try:
            ...
            if self.detection_mode == DetectionMode.PRIMARY_ONLY:
                ...
                return self._detect_with_fallback(
                    image, primary_results, quality_results
                )
    ...
    def _assess_detection_quality(
        self, results: Any, image: np.ndarray
    ) -> List[QualityMetrics]:
        """
        评估检测质量
    ...
    def _process_primary_results(
        self, results: Any, quality_results: List[QualityMetrics]
    ...
    def _detect_with_fallback(
        ...
        image: np.ndarray,
        primary_results: Any,
        quality_results: List[QualityMetrics],
    ) -> List[HandDetectionResult]:
        """
        使用备用检测器的检测策略
        ...
        """
        # 处理主检测器结果
        primary_detections = self._process_primary_results(
            primary_results, quality_results
        )

        # 检查质量是否可接受
        if self.quality_assessor.is_quality_acceptable(
            quality_results, self.quality_threshold
        ):
    ...
    def _detect_with_fusion(
        ...
        # 融合结果
        fused_results = self._fuse_detection_results(
            primary_detections, fallback_regions, image
        )
    ...
    def _fuse_detection_results(
        ...
        for detection in primary_detections:
            if (
                ...
                and detection.quality_metrics.overall_quality >= self.quality_threshold
            ):
                ...
                if best_match_idx is not None:
                    ...
                    fused_detection = HandDetectionResult(
                        ...
                        bbox=self._merge_bboxes(
                            primary_det.bbox, fallback_region["bbox"]
                        ),
                        confidence=(
                            primary_det.confidence + fallback_region["confidence"]
                        )
                        / 2,
    ...
    def get_detection_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_detections"] > 0:
            stats["primary_success_rate"] = (
                stats["primary_success"] / stats["total_detections"]
            )
            stats["fallback_usage_rate"] = (
                stats["fallback_used"] / stats["total_detections"]
            )
            stats["quality_failure_rate"] = (
                stats["quality_failures"] / stats["total_detections"]
            )
