...
class AdvancedMotionTracker:
    ...
    def _count_direction_changes(
        self, positions: np.ndarray, threshold: float = 0.1
    ) -> int:
        """计算方向变化次数"""
        if len(positions) < 3:
            return 0
    ...
    def _get_frequency_features(self) -> Dict[str, float]:
        ...
        # 主导频率
        dominant_freq_x = (
            positive_freqs[np.argmax(magnitude_x)] if len(magnitude_x) > 0 else 0.0
        )
        dominant_freq_y = (
            positive_freqs[np.argmax(magnitude_y)] if len(magnitude_y) > 0 else 0.0
        )

        # 频谱质心
        spectral_centroid_x = self._calculate_spectral_centroid(
            positive_freqs, magnitude_x
        )
        spectral_centroid_y = self._calculate_spectral_centroid(
            positive_freqs, magnitude_y
        )

        # 频谱滚降点
        spectral_rolloff_x = self._calculate_spectral_rolloff(
            positive_freqs, magnitude_x
        )
        spectral_rolloff_y = self._calculate_spectral_rolloff(
            positive_freqs, magnitude_y
        )
    ...
    def _calculate_spectral_rolloff(
        self, freqs: np.ndarray, magnitudes: np.ndarray, rolloff_ratio: float = 0.85
    ) -> float:
        """计算频谱滚降点"""
        if len(magnitudes) == 0:
            return 0.0
    ...
    def _calculate_pause_ratio(
        self, speeds: np.ndarray, pause_threshold: float = 0.01
    ) -> float:
        """计算暂停比例"""
        if len(speeds) == 0:
            return 0.0
    ...
    def _get_anomaly_features(self) -> Dict[str, float]:
        """获取异常检测特征"""
        if len(self.position_history) < 10:
            return {"anomaly_score": 0.0, "outlier_ratio": 0.0}
        ...
        return {"anomaly_score": anomaly_score, "outlier_ratio": outlier_ratio}
...
class AdaptiveThresholdManager:
    ...
    def update_thresholds(
        self, features: Dict[str, Any], detection_result: Dict[str, float]
    ):
        """
        根据新的特征和检测结果更新阈值
    ...
    def _smooth_update(
        self, current_value: float, target_value: float, rate: float
    ) -> float:
        """平滑更新阈值"""
...
class EnhancedMotionAnalyzer:
    ...
    def __init__(self):
        ...
        self.hand_trackers = (
            {}
        )  # track_id -> {'left': AdvancedMotionTracker, 'right': AdvancedMotionTracker}
    ...
    def analyze_handwashing_enhanced(self, track_id: int) -> float:
        ...
        for hand_label, tracker in trackers.items():
            ...
            if hand_conf > 0:
                ...
                logger.debug(
                    f"Hand {hand_label} enhanced handwash confidence: {hand_conf:.3f}"
                )
    ...
    def _evaluate_handwash_enhanced(
        self, features: Dict[str, Any], thresholds: Dict[str, float]
    ) -> float:
        """使用高级特征评估洗手行为"""
        confidence = 0.0
        ...
        if features["position_variance_x"] + features[
            "position_variance_y"
        ] >= thresholds.get("min_position_variance", 0.0005):
            confidence += 0.15

        # 轨迹形状特征（权重：0.3）
        if features["trajectory_length"] >= thresholds.get(
            "min_trajectory_length", 0.1
        ):
            confidence += 0.1
    ...
    def analyze_sanitizing_enhanced(self, track_id: int) -> float:
        ...
        if motion_confidences:
            ...
            distance_factor = 1.0 - (
                distance / thresholds.get("max_hand_distance", 0.15)
            )
            ...
            if combined_features:
                avg_features = {
                    key: np.mean(values) for key, values in combined_features.items()
                }
    ...
    def _evaluate_sanitize_enhanced(
        self, features: Dict[str, Any], thresholds: Dict[str, float]
    ) -> float:
        """使用高级特征评估消毒行为"""
        confidence = 0.0
        ...
        # 基础运动特征（权重：0.4）
        if (
            thresholds.get("min_avg_speed", 0.005)
            <= features["avg_speed"]
            <= thresholds.get("max_avg_speed", 0.3)
        ):
            confidence += 0.2
