...
class DataCollector:
    ...
    def update_detection(
        ...
        for behavior_type, confidence in behavior_results.items():
            if behavior_type in ["handwashing", "sanitizing"]:
                self._handle_positive_behavior(
                    track_id, frame, behavior_type, confidence, frame_metadata
                )
            else:
                self._handle_other_behavior(
                    track_id, frame, behavior_type, confidence, frame_metadata
                )
    ...
    def _handle_positive_behavior(
        ...
        if confidence >= self.confidence_threshold:
            ...
            if behavior_type not in self.active_segments[track_id]:
                ...
                logger.debug(
                    f"Started recording {behavior_type} segment for track {track_id}"
                )
    ...
    def _finalize_segment(self, track_id: int, behavior_type: str):
        """完成并保存视频片段"""
        if (
            track_id not in self.active_segments
            or behavior_type not in self.active_segments[track_id]
        ):
            return
        ...
        if segment.get_duration() >= self.min_segment_duration:
            ...
            try:
                ...
                logger.info(
                    f"Queued {behavior_type} segment {segment.segment_id} "
    ...
    def _save_segment(self, segment: VideoSegment):
        ...
        try:
            ...
            # 保存标注信息
            annotation_path = (
                self.data_root / "annotations" / f"{segment.segment_id}.json"
            )
    ...
    def _save_keypoints(self, segment: VideoSegment, keypoints_path: Path):
        """保存关键点数据"""
        keypoints_data = {"segment_id": segment.segment_id, "frames": []}
    ...
    def get_stats(self) -> Dict:
        ...
        stats["active_segments"] = sum(
            len(segments) for segments in self.active_segments.values()
        )
    ...
    def create_video_clip(
        self, behavior_type: str, track_id: int, confidence: float
    ) -> str:
        """
        创建新的视频片段
        ...
        logger.debug(
            f"Created {behavior_type} segment {segment_id} for track {track_id}"
        )
        return segment_id
