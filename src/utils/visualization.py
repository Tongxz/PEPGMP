...
class VisualizationManager:
    ...
    def visualize_frame(
        ...
        try:
            ...
            if "hand_results" in results and self.config["show_bbox"]:
                vis_frame = self._draw_hand_detections(
                    vis_frame, results["hand_results"]
                )
            ...
            if "motion_results" in results and self.config["show_behavior_text"]:
                vis_frame = self._draw_behavior_info(
                    vis_frame, results["motion_results"]
                )

            # 绘制统计信息
            if self.config["show_statistics"]:
                vis_frame = self._draw_statistics(
                    vis_frame, results, frame_count, total_frames
                )
            ...
            vis_frame = self.cv2_add_chinese_text(
                vis_frame,
                error_text,
                (20, height - 30),
                font_size=24,
    ...
    def _draw_hand_detections(
        self, frame: np.ndarray, hand_results: List[Any]
    ) -> np.ndarray:
        """
        绘制手部检测边界框
        ...
        for i, hand in enumerate(hand_results):
            ...
            if bbox:
                ...
                for j in range(1, len(points)):
                    cv2.line(
                        frame, points[j - 1], points[j], self.colors["trajectory"], 2
                    )

        return frame

    def _draw_behavior_info(
        self, frame: np.ndarray, motion_results: Dict[str, Any]
    ) -> np.ndarray:
        """
        绘制行为识别信息
    ...
    def _draw_frame_info(
        self, frame: np.ndarray, frame_count: int, total_frames: Optional[int] = None
    ) -> np.ndarray:
        """
        绘制帧信息
    ...
    def draw_control_hints(self, frame: np.ndarray) -> np.ndarray:
        ...
        hints = ["控制键:", "空格键: 暂停/继续", "ESC键: 退出", "S键: 保存帧", "R键: 重置统计"]
