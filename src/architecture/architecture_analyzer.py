...
class ArchitectureAnalyzer:
    ...
    def analyze_project(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dependencies": [
                self._dependency_to_dict(dep) for dep in self.dependencies
            ],
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            if module_name:
                self.modules[module_name] = ModuleInfo(
                    name=module_name, path=str(py_file)
                )
                    ...
                    if isinstance(node, ast.Import):
                        for alias in node.names:
                            self._process_import(
                                module_name, alias.name, "direct", node.lineno
                            )
                    elif isinstance(node, ast.ImportFrom):
                        if node.module:
                            self._process_import(
                                module_name, node.module, "relative", node.lineno
                            )
    ...
    def _process_import(
        self, source_module: str, target_module: str, import_type: str, line_number: int
    ):
        """处理导入语句"""
        # 清理模块名
    ...
    def _calculate_coupling_scores(self):
        ...
        for module_name, module_info in self.modules.items():
            ...
            # 计算复杂度：代码行数 + 依赖数量
            complexity_score = (
                module_info.lines_of_code + len(module_info.dependencies) * 10
            )
    ...
    def _calculate_architecture_metrics(self) -> ArchitectureMetrics:
        ...
        if self.modules:
            coupling_scores = [
                module.coupling_score for module in self.modules.values()
            ]
        ...
        metrics.architecture_complexity = (
            metrics.circular_dependencies * 10 + metrics.high_coupling_modules * 5
        )
    ...
    def _generate_recommendations(self) -> List[Dict[str, Any]]:
            ...
            recommendations.append(
                {
                    ...
                    "suggestions": ["使用依赖注入模式", "引入抽象层", "重构模块职责"],
                }
            )
        ...
        high_coupling_modules = [
            name
            for name, module in self.modules.items()
            if module.coupling_score
            > self._calculate_architecture_metrics().average_coupling * 1.5
        ]
            ...
            recommendations.append(
                {
                    ...
                    "suggestions": ["使用接口抽象", "实现依赖倒置", "拆分大型模块"],
                }
            )

        # 3. 复杂模块建议
        complex_modules = [
            (name, module.complexity_score)
            for name, module in self.modules.items()
            ...
            recommendations.append(
                {
                    ...
                    "suggestions": ["拆分功能职责", "提取公共组件", "使用设计模式"],
                }
            )
