        ...
        for attempt in range(handler.max_retries):
            try:
                ...
                if attempt < handler.max_retries - 1:
                    logger.warning(f"处理器重试 {attempt + 1}/{handler.max_retries}: {handler.handler_id}")
    ...
    async def _execute_handler_async(self, handler: EventHandler, event: Event):
        ...
        for attempt in range(handler.max_retries):
            try:
                ...
                if attempt < handler.max_retries - 1:
                    logger.warning(f"异步处理器重试 {attempt + 1}/{handler.max_retries}: {handler.handler_id}")
...
class DetectionEvent(Event):
    ...
    def __init__(self, detection_type: str, **kwargs):
        super().__init__(
            event_type=f"detection.{detection_type}",
            **kwargs
        )
...
class ModelEvent(Event):
    ...
    def __init__(self, model_action: str, **kwargs):
        super().__init__(
            event_type=f"model.{model_action}",
            **kwargs
        )
...
class SystemEvent(Event):
    ...
    def __init__(self, system_action: str, **kwargs):
        super().__init__(
            event_type=f"system.{system_action}",
            **kwargs
        )
...
class BaseEventHandler(ABC):
    ...
    def register(self, event_types: Union[str, List[str]], priority: EventPriority = EventPriority.NORMAL):
        """注册处理器"""
...
if __name__ == "__main__":
    ...
    detection_event = DetectionEvent(
        ...
        data={"detection_count": 5, "confidence": 0.95}
    )
    ...
    model_event = ModelEvent(
        model_action="loaded",
        data={"model_path": "/path/to/model.pt", "size": "100MB"}
    )
