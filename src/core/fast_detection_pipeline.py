...
class BatchDetector:
    ...
    def _process_batch(self, batch: List[np.ndarray]) -> List[FastDetectionResult]:
        ...
        logger.info(
            f"批量处理完成: {len(batch)}帧, {batch_time*1000:.1f}ms, {len(batch)/batch_time:.1f} FPS"
        )
...
class FastDetectionPipeline:
    ...
    def detect(self, frame: np.ndarray) -> FastDetectionResult:
        """同步检测接口"""
        future = asyncio.run_coroutine_threadsafe(self.detect_async(frame), self.loop)
...
async def test_fast_pipeline():
    ...
    for i in range(100):
        ...
        if i % 10 == 0:
            ...
            logger.info(
                f"帧 {i}: {processing_time*1000:.1f}ms, "
