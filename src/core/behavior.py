...
class BehaviorRecognizer:
    ...
    def __init__(  # noqa: C901
        ...
        if self.use_advanced_detection:
            try:
                from src.detection.deep_behavior_recognizer import (
                    DeepBehaviorRecognizer,
                )
                        ...
                        if self.temporal_smoother and track_id is not None:
                            ...
                            if keypoints is not None and confidences is not None:
                                ...
                                (
                                    ...
                                    str(track_id), keypoints, confidences
                                )
                                ...
                                for detection in pose_data:
                                    if "keypoints" in detection:
                                        detection["keypoints"][
                                            "xy"
                                        ] = smoothed_keypoints.tolist()
                                        detection["keypoints"][
                                            "conf"
                                        ] = smoothed_confidences.tolist()
                                        break
