...
class HandDetectionQualityAssessor:
    ...
    def assess_quality(self, results: Any, image: np.ndarray) -> List[QualityMetrics]:
        """
        评估手部检测质量
    ...
    def _extract_landmark_points(
        self, landmarks: Any, image_shape: Tuple[int, int, int]
    ) -> np.ndarray:
        """
        提取关键点坐标
        ...
        for i, landmark in enumerate(landmarks.landmark):
            points[i] = [landmark.x * w, landmark.y * h, landmark.z]  # 相对深度
    ...
    def _check_connection_validity(self, landmark_points: np.ndarray) -> float:
        ...
        try:
            ...
            return (
                valid_connections / total_connections if total_connections > 0 else 0.0
            )
    ...
    def _calculate_motion_continuity(self, landmark_points: np.ndarray) -> float:
        ...
        try:
            ...
            for i in range(len(current_motion)):
                if (
                    np.linalg.norm(current_motion[i]) > 0.1
                    and np.linalg.norm(prev_motion[i]) > 0.1
                ):
                    # 计算余弦相似度
                    cos_sim = np.dot(current_motion[i], prev_motion[i]) / (
                        np.linalg.norm(current_motion[i])
                        * np.linalg.norm(prev_motion[i])
                    )
    ...
    def is_quality_acceptable(
        self, quality_metrics: List[QualityMetrics], threshold: float = 0.7
    ) -> bool:
        """
        判断检测质量是否可接受
