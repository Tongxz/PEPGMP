...
class ModelQuantizer:
    ...
    def quantize_model_dynamic(
        ...
        try:
            ...
            logger.info(
                f"Model {model_name} quantized: {original_size:.2f}MB -> {quantized_size:.2f}MB "
    ...
    def quantize_model_static(
        self, model: "nn.Module", model_name: str, calibration_data: List[torch.Tensor]
    ) -> "nn.Module":
        """静态量化模型
    ...
    def benchmark_model(
        ...
        speedup = (
            avg_original_time / avg_quantized_time if avg_quantized_time > 0 else 1.0
        )
    ...
    def __init__(
        ...
        self.pools = {
            ...
            "recognition": ThreadPoolExecutor(
                max_workers=recognition_workers, thread_name_prefix="recognition"
            ),
        ...
        logger.info(
            f"ThreadPoolManager initialized with {sum(pool._max_workers for pool in self.pools.values())} total workers"
        )
        ...
        def execute_task():
            ...
            try:
                ...
                with self.lock:
                    ...
                    self.task_stats["avg_processing_time"] = np.mean(
                        self.processing_times
                    )
    ...
    def get_pool_stats(self) -> Dict[str, Any]:
        ...
        for name, pool in self.pools.items():
            stats[name] = {
                ...
                "queue_size": self.task_queues[name].qsize()
...
class PerformanceOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"PerformanceOptimizer initialized with optimizations: "
    ...
    def optimize_model(
        ...
        if calibration_data:
            return self.quantizer.quantize_model_static(
                model, model_name, calibration_data
            )
        else:
    ...
    def benchmark_optimization(
        ...
        results["summary"] = {
            ...
            "frames_per_second_async": len(test_frames) / avg_async_time
            if avg_async_time > 0
            else 0,
        }
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["frame_processing_times"]:
            metrics["avg_frame_processing_time"] = np.mean(
                metrics["frame_processing_times"]
            )
    ...
    def auto_tune_parameters(
        self, target_fps: float = 30.0, sample_frames: List[np.ndarray] = None
    ) -> Dict[str, Any]:
        """自动调优参数
