...
class UserProfile:
    ...
    def update_behavior_stats(
        self, behavior_type: str, duration: float, confidence: float
    ):
        """更新行为统计
    ...
    def add_feedback(self, feedback_type: str, context: Dict[str, Any]):
        ...
        Args:
            ...
            context: 反馈上下文信息
        """
        feedback = {"type": feedback_type, "timestamp": time.time(), "context": context}
...
class AdaptiveThresholdOptimizer:
    ...
    def optimize_thresholds(
        self, user_profile: UserProfile, recent_performance: Dict[str, float]
    ) -> Dict[str, float]:
        """优化检测阈值
    ...
    def _adjust_threshold(
        self, base_threshold: float, sensitivity: float, precision: float, recall: float
    ) -> float:
        """调整单个阈值"""
        # 目标是平衡精确率和召回率
...
class BehaviorPatternAnalyzer:
    ...
    def analyze_motion_patterns(
        self, motion_sequences: List[np.ndarray]
    ) -> Dict[str, Any]:
        """分析运动模式
...
class PersonalizationEngine:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        """
        self.storage_path = (
            Path(storage_path) if storage_path else Path("user_profiles")
        )
        ...
        logger.info(
            f"PersonalizationEngine initialized with {len(self.user_profiles)} user profiles"
        )
    ...
    def _extract_behavior_features(
        self, motion_data: Dict[str, Any], behavior_type: str
    ) -> np.ndarray:
        """从运动数据中提取行为特征"""
        features = []
    ...
    def _calculate_recent_performance(self, profile: UserProfile) -> Dict[str, float]:
        ...
        def calculate_metrics(feedback_list):
            ...
            false_positive = len(
                [f for f in feedback_list if f["type"] == "false_positive"]
            )
            false_negative = len(
                [f for f in feedback_list if f["type"] == "false_negative"]
            )
    ...
    def _calculate_false_positive_rate(self, profile: UserProfile) -> float:
        ...
        false_positives = len(
            [f for f in recent_feedback if f["type"] == "false_positive"]
        )
