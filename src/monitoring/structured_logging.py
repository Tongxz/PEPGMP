...
class LogAnalyzer:
    ...
    def analyze_logs(self, 
        ...
        # 错误分析
        error_entries = [e for e in filtered_entries if e.level in [LogLevel.ERROR, LogLevel.CRITICAL]]
        error_patterns = self._analyze_error_patterns(error_entries)
        
        # 性能分析
        performance_entries = [e for e in filtered_entries if e.category == LogCategory.PERFORMANCE]
    ...
    def _analyze_performance(self, performance_entries: List[LogEntry]) -> Dict[str, Any]:
        """分析性能日志"""
...
class LogVisualizer:
    ...
    def _generate_trends(self, start_time: Optional[float], end_time: Optional[float]) -> Dict[str, Any]:
        """生成趋势数据"""
        # 按小时分组统计
        hourly_data = {}
    ...
    def _generate_hotspots(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        """生成热点分析"""
        hotspots = {
            "error_hotspots": analysis.get("error_analysis", {}).get("most_common_errors", [])[:5],
