...
from ...utils.error_handler import get_error_handler, ErrorSeverity, ErrorCategory
from ...utils.error_monitor import get_error_monitor, get_health_checker, AlertLevel
...
async def get_errors_by_category(
    category: str,
    limit: int = Query(50, ge=1, le=1000, description="返回错误数量限制")
):
    """根据错误分类获取错误列表"""
    try:
        # 验证分类
        ...
        return {
            "category": category,
            "total_count": len(error_handler.error_tracker.get_errors_by_category(error_category)),
...
async def get_errors_by_severity(
    ...
    try:
        ...
        return {
            "severity": severity,
            "total_count": len(error_handler.error_tracker.get_errors_by_severity(error_severity)),
            "returned_count": len(error_list),
            "errors": error_list
        }
...
async def resolve_alert(alert_id: str):
    ...
    try:
        ...
        return {
            ...
            "alert_id": alert_id,
            "resolved_at": error_monitor.active_alerts.get(alert_id, {}).get("resolved_at")
        }
...
async def get_performance_stats():
    ...
    try:
        ...
        return {
            "message": "性能统计功能开发中",
            "timestamp": error_monitor.active_alerts.get(alert_id, {}).get("resolved_at")
        }
