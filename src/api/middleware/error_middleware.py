...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    def _handle_http_exception(self, exception: HTTPException, context: ErrorContext, start_time: float):
        """处理HTTP异常"""
        response_time = time.time() - start_time
        ...
        logger.warning(f"HTTP异常: {context.request_id}, 状态码: {exception.status_code}, 响应时间: {response_time:.3f}s")
    
    def _handle_general_exception(self, exception: Exception, context: ErrorContext, start_time: float):
        """处理一般异常"""
        response_time = time.time() - start_time
        ...
        logger.error(f"请求异常: {context.request_id}, 错误: {str(exception)}, 响应时间: {response_time:.3f}s")
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if client_ip in self.blocked_ips:
            return JSONResponse(
                status_code=403,
                content={"error": "IP被阻止", "message": "您的IP地址已被阻止访问"}
            )
    ...
    def _check_rate_limit(self, client_ip: str) -> bool:
        ...
        if self.request_counts[client_ip] > self.rate_limit_threshold:
            self._log_security_event("rate_limit_exceeded", client_ip, f"请求数: {self.request_counts[client_ip]}")
            return False
    ...
    def _check_suspicious_activity(self, request: Request, response: Response):
        ...
        if response.status_code >= 500:
            self._log_security_event("server_error", client_ip, f"状态码: {response.status_code}")
        
        # 检查异常请求路径
        if ".." in str(request.url.path) or "admin" in str(request.url.path).lower():
            self._log_security_event("suspicious_path", client_ip, f"路径: {request.url.path}")
