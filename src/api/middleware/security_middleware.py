...
from ...security.security_manager import SecurityLevel, ThreatType, get_security_manager
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if not self._check_access_permission(request, client_ip):
                ...
                return JSONResponse(
                    status_code=403, content={"error": "访问被拒绝", "message": "您没有权限访问此资源"}
                )
    ...
    def _post_process_security_check(
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self._is_protected_path(request_path):
            ...
            if not self._authenticate_request(request):
                return JSONResponse(
                    status_code=401, content={"error": "未授权", "message": "请先登录"}
                )
...
class CSRFProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        request_path = str(request.url.path)
        if any(
            request_path.startswith(exempt_path)
